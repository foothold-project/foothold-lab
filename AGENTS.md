...
6. **회의록 파일은 참석 멤버로 가른다.**
   - 멤버가 같으면 같은 md에 이어 쓴다. 날짜가 달라도, 주제가 달라도 합친다.
   - 멤버가 다르면 다른 md다. 같은 날이어도 파일을 나누다.
   - 날짜·주제로 쪼개지 않는다.

7. **공부·참고하라고 하면 파일을 만들지 않는다.**
   - 읽고 이후 작업에 쓴다. md로 제출하지 않는다.
   - 정리본·제출 파일은 사용자가 만들라고 한 때만 만든다.
