...
python refresh.py                         # 빌드 · PDF · 검증을 한 번에 한다
```

`refresh.py` 는 빌드하고 PDF 를 뽑은 뒤 **산출물을 다시 열어서** 열 가지를 잰다.
심은 영상이 저장소 사본과 바이트가 같은지, 문서가 완전한지, 외부 참조가 0건인지,
PDF 가 19쪽 960x540 인지, 빈 쪽이 없는지, 한글이 성한지 따위다. 하나라도 어긋나면
0 이 아닌 값으로 끝난다.

「빌드가 돌았다」는 성공의 증거가 아니다. 2026-09-04 에 빌드가 예외로 죽었는데 앞선
산출물이 그대로 남아 있어서 「바뀐 것이 없다」를 「같게 나왔다」로 읽을 뻔했다.

빌드만 하려면 `python slides.py` 를 쓴다.
