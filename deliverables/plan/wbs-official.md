...
**관문은 기한과 다음 단계 착수 조건을, 구현 단계는 작업 순서와 선행 관계를 정의한다.** 지금은 「A-정책 09/12 안에 이슈 몇 건」만 있고 그 이슈들이 어떤 차례로 이어지는지가 없다. 8/31 멘토링이 정한 「평지 기준선을 먼저, 그다음 실패 지형」이 바로 그 차례인데 WBS 어디에도 적혀 있지 않았다.
...
2026-08-27 설계(`deliverables/plan/wbs.md`)는 3층이었다. 위 두 층을 사람이 정하고 세부 작업을 GitHub 이슈에서 생성하는 구조다. **작업 순서를 나타내는 구현 단계 분류를 추가해 네 층으로 구성한다.**

| 층 | 무엇 | 누가 정하나 | 개수 | 정본 |
|---|---|---|---|---|
| 1 | 작업 영역 | 사람 · 고정 | 8 | `docs/ROLES.md` |
| 2 | 관문 | 사람 · 날짜 | 5 | `tools/build_wbs.py` 의 `GATES` |
| **3** | **구현 단계** | **사람 · 순서** | **트랙마다 셋 또는 넷** | **이 문서 5절 · 팀장 확정 전** |
| 4 | 세부 작업 | **이슈에서 자동** | 계속 늘어난다 | GitHub 이슈 |

**왜 층을 나누나.** 이슈 제목을 그대로 WBS 에 올리면 사람마다 쓰는 말이 달라 표가 흩어진다. 작업 영역·관문·구현 단계를 기준으로 분류하면 서로 다른 제목의 이슈도 같은 기준으로 집계할 수 있다. 위 세 층은 이슈가 하나도 없어도 그려지고, 4층은 진척을 채우는 데 쓴다.

**3층이 1층과 다른 점.** 1층은 「누가 담당하는가」라는 책임 구분이고 3층은 「무엇 다음에 무엇인가」라는 시간의 축이다. 한 작업 영역이 여러 단계에 걸치고, 한 단계에 여러 작업 영역이 함께 들어간다.

---

## 3. 1층 · 작업 영역 8갈래

`docs/ROLES.md` 가 정본이다. 아래 표는 해당 문서의 담당 배치를 인용한다.
...
**주담당은 해당 영역의 문제 해결과 결과 보고를 책임진다.** 부담당은 검토와 대체를 맡는다. 한 사람이 자리를 비워도 갈래가 멈추지 않도록 갈래마다 두 명 이상을 둔다.
...
기획서 8-1 이 정본이고, 날짜는 `tools/build_wbs.py` 의 `GATES` 에서 읽는다. **관문 통과는 기한 도래가 아니라 다음 단계 착수 조건 충족으로 판정한다.** 조건을 채우지 못하면 다음 단계에 착수하지 않는다.
...
**아래 단계 이름과 개수는 이슈 #122 의 초안이고 팀장 확정 전이다.** 기획서의 두 트랙 축을 그대로 따랐다. 「관련 관문」 열은 기획서 8-2 의 각 작업 항목에 이미 붙어 있는 관문을 모은 것이고, 새로 만든 날짜가 아니다.

| 트랙 | 단계 | 무엇을 하나 | 관련 관문 | 기획서 8-2 의 해당 작업 항목 |
|---|---|---|---|---|
| Track A | 1 · 기준선 확립 | 평지와 미경험 10종을 지형별로 정한 조건에 따라 재측정하고 하네스를 고정한다 | A-정책 | 기본값 1,500 iter 학습 지형 결과 분석 · 평가 하네스 확장 7건 · 지표 3종 정본화 |
...
| 단계 | 완료 판정 | 값의 성격 |
|---|---|---|
| Track A 1 · 기준선 확립 | 평지 100 에피소드에서 낙상 없이 통과선 10 m 도달 100% 유지 · 통과선 지점 좌우 이탈 5 cm 이내를 90% 이상으로 올린다 | 우리 측정 위에 세운 목표 |
| Track A 2 · 실패 지형 진단 | 실패 5종 각각 난이도 열 칸 · 칸당 100 에피소드를 돌려 경계를 낸다 | **목표 이동폭은 비어 있다.** 경계값 자체가 아직 없다 |
...
지금은 이 문서에만 있고 생성기에는 없다. **이슈 라벨 방식은 기존 조회 기능을 재사용할 수 있다.**

| 방법 | 어떻게 | 장점 | 단점 |
|---|---|---|---|
| **이슈 라벨** | `S/A1-기준선` 처럼 단계 라벨을 생성한다 | `tools/build_wbs.py` 가 이미 라벨을 읽는다. `AREAS` 옆에 `STAGES` 를 두고 같은 방식으로 찾으면 된다 | 라벨이 여덟 개 더 늘어난다. 붙이는 것을 잊으면 `(미분류)` 가 된다 |
| 프로젝트 보드 필드 | 보드에 단계 필드를 만든다 | 라벨 목록이 안 늘어난다 | 생성기가 지금 보드를 안 읽는다. 이슈 목록만 읽는다. 읽는 경로를 새로 만들어야 한다 |

**기존 생성기를 활용할 수 있는 이슈 라벨 방식을 제안한다.** 단계 이름은 팀장 확정 후 라벨로 생성한다. 라벨은 한 번 붙으면 이슈 전체를 다시 손봐야 바뀐다.
...
위 통과율은 난이도 0.5 한 점에서 6초 · 통과선 3 m · 1.0 m/s · 지형당 100 에피소드 · seed 42 · 시험 중 최대 좌우 이탈 임계 1.5 m 로 평가한 값이다. **같은 통과율이라도 관측된 실패 양상은 다르다.** `stepping_stones(디딤돌)` 과 `gap(틈)` 은 낙상이 주요 실패 양상이며, `floating_ring(부유 고리)` 은 전진 미달이다. 형상과 정책의 영향을 구분하기 위해 난이도 격자별 통과 여부를 먼저 측정한다.
...
| 축 | 무엇을 보나 | 어디서 오나 | 성격 |
|---|---|---|---|
| 1 · 영역 책임 | 여덟 갈래 중 어느 것의 주담당이고 어느 것의 부담당인가 | `docs/ROLES.md` | 사람이 정한다 · 거의 안 바뀐다 |
| 2 · 실패 지형 담당 | 실패 5종 중 어느 하나를 맡고 분석과 설정안을 냈는가 | 기획서 8-3 · 이슈 다섯 건 | 사람이 정한다 · 1인 1지형 |
| 3 · 작업 실적 | 담당자로 걸린 이슈가 어느 영역 · 어느 관문 · 어느 단계에 몇 건이고 그중 몇 건이 닫혔는가 | GitHub 이슈의 `assignees` 와 `state` | **GitHub 이슈에서 자동 집계 · 매주 바뀐다** |
...
**세지 않는 것도 정해 둔다.** 커밋 수와 코드 줄 수는 기여 축으로 쓰지 않는다. 갈래마다 산출물의 단위가 다르기 때문이다. `C/기록` 의 산출물은 문서고 `A/트윈렌더` 의 산출물은 씬과 렌더며 `A/정책학습` 의 산출물은 학습 로그와 체크포인트다. 산출물 유형이 달라 커밋 수나 코드 줄 수를 단순 비교하면 기여도를 왜곡할 수 있다.
...
| 그 파일이 언제 갱신되나 | `.github/workflows/weekly-report.yml` | 매주 월요일 주간 리포트와 함께 갱신한다. 바뀐 것이 있을 때만 커밋한다 |
...
**미확정 항목과 확정에 필요한 조건·시점을 정리한다.**

| 빈 자리 | 무엇이 있어야 채워지나 | 언제 |
|---|---|---|
| 구현 단계의 이름과 개수 | 팀장 확정. 5-1 은 이슈 #122 의 초안이다 | 기획발표 09/04 전후 |
| 단계 라벨 | 위 확정. 확정 전에 라벨을 생성하면 이슈 전체를 다시 손봐야 한다 | 단계 확정 직후 |
| 생성기의 3층 지원 | `tools/build_wbs.py` 에 `STAGES` 추가. 이슈 #122 의 후속 작업 | 라벨 생성 뒤 |
| 단계별 기여 집계 | 위 셋이 모두 갖춰진 뒤 | 라벨이 이슈에 붙은 뒤 |
| Track A 단계 2 · 4 의 목표값 | 실패 5종 각각 난이도 열 칸 · 칸당 100 에피소드. 지금까지의 1,000 에피소드는 난이도 0.5 한 점이라 경계를 만들지 못한다 | A-정책 09/12 |
...
| 판 | 언제 | 무엇이 바뀌었나 | 근거 |
|---|---|---|---|
| **v1.1** | 2026-09-03 | #171 공식 제출 문체 정리. 관문·구현 단계 정의와 GitHub 자동 집계 표현 명확화. 미확정 단계는 초안으로 유지하고 기존 정비 목록을 부록으로 명시. PDF 판 표기 제외 | #171 |
