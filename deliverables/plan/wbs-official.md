...
위 통과율은 난이도 0.5 한 점에서 6초 · 통과선 3 m · 1.0 m/s · 지형당 100판 · seed 42 · 시험 중 최대 좌우 이탈 임계 1.5 m 로 평가한 값이다. **같은 통과율이라도 관측된 실패 양상은 다르다.** `stepping_stones(디딤돌)` 과 `gap(틈)` 은 낙상이 주요 실패 양상이며, `floating_ring(부유 고리)` 은 전진 미달이다. 형상과 정책의 영향을 구분하기 위해 난이도 격자별 통과 여부를 먼저 측정한다.
