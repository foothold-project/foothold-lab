...
각 기술을 이름으로 나열하지 않고 **무엇을 고쳐 쓰는지**로 적는다. 대부분 기성 구현체를 가져다 쓰되, 우리 문제에 맞게 손대는 지점이 어디인지가 이 프로젝트의 실질이다.

### Track A · 시뮬레이션과 강화학습

| 활용 환경 · 커스터마이징 요소 | 개발 내용 |
|---|---|
| **NVIDIA Isaac Sim · Isaac Lab v2.3.2** | Isaac Lab 이 제공하는 Unitree Go2 locomotion 환경과 학습 코드를 기반으로 시뮬레이션·강화학습 환경을 구성한다. 4,096 병렬 환경에서 21,248~21,385 steps/s, 표준 1,500 iteration 이 약 113분이다 (RTX 5080 1대 · 2026-08-05 실측) |
...
| **Reward Function** | 속도 추종·자세 안정성·토크·action 변화량·발 접촉 등 Isaac Lab 이 제공하는 reward 의 가중치와 조건을 수정한다. **현재 정책은 관절이 63.7 cm 까지 발을 들 수 있는데 3.25 cm 만 든다.** `feet_air_time` 이 높이가 아니라 시간을 점수로 주고, 발 높이 항이 없으며, `flat_orientation_l2` 가중치가 0 이기 때문이다. 여기가 실패 5종의 공통 뿌리다 |
...
| **파인튜닝과 망각 방지** | 특정 지형만 추가 학습하면 이미 되던 지형이 후퇴한다. 재학습 시 기존 rough 지형 0.6 과 대상 지형 0.4 를 섞어 회귀를 막는다 |
| **MuJoCo** | 결과가 시뮬레이터 특유의 성질에서 온 것인지 확인하는 독립 2차 검증 환경 `계획` |

**Track A 의 반복 흐름** · Baseline 평가 → 실패 지형 분석 → 학습 환경·보상 개선 → 재학습 → 미경험 지형 재평가. 높은 reward 를 얻는 것이 목적이 아니라 **어떤 학습 지형과 보상이 새 지형의 보행에 영향을 주는지**를 가리는 것이 목적이다.
...
| **COLMAP** | 특징점 추출·이미지 정합·카메라 pose 추정을 활용해 3D Gaussian Splatting 에 필요한 카메라 정보와 초기 geometry 를 생성한다 |
| **3D Gaussian Splatting** | 기존 3DGS 구현체로 대상 공간의 시각적 형상을 복원하고, 촬영 조건과 reconstruction parameter 를 조정해 품질을 개선한다 |
...
**시각적으로 닮은 것과 물리적으로 검증 가능한 것은 다르다.** 3DGS 복원물과 보행용 충돌 형상을 두 층으로 갈라 각각 구축·검증한다.
...
명령 속도를 0.5 에서 1.0 m/s 로 두 배 올린 평가에서도 **5종 통과·5종 실패로 분류됐다.** 다만 이 비교만으로 실패 원인을 지형 자체로 단정할 수는 없다. 정책과 지형 조건의 영향을 추가로 분석해야 한다.
...
| **RL Locomotion**<br>보조 지표 | 지형별 병목 해소 | 실패 5종 종합 통과율 평균 **1.4% → 30% 이상**<br>낙상형 2종 생존율 **20% → 60% 이상**<br>정체형 `rails(턱)` 속도 추종 **8% → 50% 이상**<br>전진불능형 2종 전진 성공률 **0~15% → 50% 이상** |
...
지형 난이도가 달라지면 성공률만으로 정책 성능을 비교할 수 없다. **성공률로 목표를 걸면 맵을 쉽게 만들 유인이 생기고, 그 순간 학습 전후 비교가 끊긴다.**

경계로 재면 다르다. 같은 지형의 난이도 파라미터를 단계로 나눠 **어디까지 되고 어디부터 안 되는지**를 재고, 학습 후 그 경계가 얼마나 밀렸는지를 본다. 맵을 바꿔도 비교가 이어지고, 지형 자체가 벽인 경우에도 **벽 앞에서 어디까지 가능한지**가 결과로 남는다.

예를 들어 `rails(턱)` 는 턱 높이를 0.05~0.18 m 로 준다. 현재 정책이 몇 cm 에서 무너지는지가 경계이고, 학습 후 그 경계가 몇 cm 로 올라갔는지가 성과다.

> **각 지형의 현재 경계값은 A-정책 관문(9/12)에 측정해 고정한다.** 위 「25% 이상 이동」은 그 경계 대비 상대값이다. 절대 수치를 지금 적어 두고 나중에 맞추지 않는다.
...
| 직진성 | 통과선 지점 좌우 이탈 **5 cm 이내** | 8/31 멘토링 |

**축을 나눈 이유는 실패가 지형마다 다르기 때문이다.** 6-3 에서 본 대로 어떤 지형은 넘어져서, 어떤 지형은 느려서, 어떤 지형은 못 나가서 실패한다. 하나의 성공률로 뭉치면 처방을 세울 수 없다.

Track B 의 정량 임계값 중 실기 환경에 따라 달라지는 항목(주행 구간 길이 · 장애물 배치)은 대여 장비 사양과 PoC 구간 확정 후 재검토한다.

### 7-4. 평가 기록과 재현성

모든 결과에 **조건 · 정책 체크포인트 · 지형 seed · 평가 인자 · 원자료 · 재실행 방법**을 함께 남긴다. 평가 근거에는 시연 영상과 함께 **동일 조건의 반복 평가 결과**를 포함한다. 같은 seed 로 다시 돌려 원자료가 일치하지 않으면 결과로 세지 않는다.
...
프로젝트는 **다섯 개 관문**으로 진행한다. 각 관문은 산출물과 「다음을 여는 조건」을 함께 갖는다. 조건을 못 채우면 다음 관문을 열지 않는다.

| 관문 | 일자 | 핵심 산출물 | 다음을 여는 조건 |
|---|---|---|---|
| **기획 발표** | 09/04 | 문제·범위·KPI·WBS·R&R 합의 | 측정 언어와 담당 확정 |
...
| **FINAL · 최종발표** | 12/11 | 자율 보행 시연 · 검증 보고서 | 공개 승격 가능한 최종 증거 |

### 8-2. 작업 분해 (WBS)

작업 영역은 두 트랙과 공통으로 **8갈래**다 (`docs/ROLES.md` 정본). 주담당은 「혼자 한다」가 아니라 **「막히면 이 사람에게 묻는다」** 는 뜻이다. 부담당이 리뷰와 대체를 맡아 한 사람이 빠져도 갈래가 멈추지 않게 한다.

| 갈래 | 작업 항목 | 담당 | 관문 | 산출물 |
|---|---|---|---|---|
| `A/정책학습` | 기본값 1,500 iter 학습 지형 결과 분석 | 임석헌 | A-정책 | 정체·낙상 분리 분석표 |
| `A/정책학습` | 보상 항 재설계 · 발 높이 항 신설과 재학습 | 임석헌 | MVP | 학습 로그 · 정책 체크포인트 |
| `A/정책학습` | 망각 방지 혼합 학습 (rough 0.6 · 대상 0.4) | 임석헌 | MVP | 회귀 검증표 |
| `A/정책학습` | 확장 지형 학습과 held-out 평가 | 임석헌 | NAV | 일반화 성능표 |
| `A/평가` | 평가 하네스 확장 5건 · 새 기준 측정 가능화 | 오현민 | A-정책 | 하네스 · 시험 68건 |
| `A/평가` | 지표 3종과 판정 기준 정본화 | 오현민 | A-정책 | 판정 함수 · 임계값 문서 |
| `A/평가` | 난이도 격자 스윕과 경계 측정 | 오현민 | A-정책 · MVP | 지형별 경계표 |
| `A/평가` | 재현성 결함 수정 (전역 난수 사용 4종) | 오현민 | A-정책 | 굽기 재현 검증 |
...
| `C/운영` | 일정 · 예산 · 범위 관리 · 멘토 인터페이스 | 오흥재 | 전 관문 | 일정 대시보드 |

### 8-3. 실패 지형은 한 사람이 하나씩 소유한다

실패 5종을 나눠 맡는다. **진단에서 그치지 않고 「진단 → 레시피」까지가 한 사람의 몫**이다. 레시피는 그 지형을 넘기기 위해 무엇을 바꿔야 하는지의 처방이다.
...
**모든 갈래에 담당이 있고, 모든 사람에게 실패 지형이 하나씩 있다.** 문서·발표를 맡은 사람도 실기 항법과 실패 지형을 함께 맡는다.
