...
| 개발 영역 | 핵심 목표 | KPI | 출처 |
|---|---|---|---|
| **RL Locomotion**<br>층 1 · 주 지표 | 실패 5종의 병목 축 개선 | `rails(턱)` 속도 오차 중앙값 **0.488 → 0.25 m/s 이하**<br>측정은 **낙상 판을 뺀 살아남은 판 기준**이다 (현재 85판의 중앙값 0.502)<br>`pit(구덩이)` 전진 중앙값 **1.09 m → 통과선 3 m**<br>`floating_ring(부유 고리)` 전진 중앙값 **1.07 m → 통과선 3 m**<br>`stepping_stones(디딤돌)` · `gap(틈)` 생존율은 **목표치를 격자 측정 후 확정** (현재 20% · 21%) | `#99` 재측정<br>2026-08-07 팀 결정 |
...
| **SLAM**<br>도전 목표 · 일반 기준 | Mapping 과 Localization | Localization 유지율 **90% 이상**<br>위치 오차 **30 cm 이하** | 임석헌 초안 (9/1) |
| **Nav2**<br>도전 목표 · 일반 기준 | 실기 Go2 자율 보행 | **20회 이상** 반복 주행<br>목표 지점 도달률 **80% 이상**<br>충돌 없는 주행률 **90% 이상**<br>경로 재계획 성공률 **80% 이상** | 임석헌 초안 (9/1) |
| **Computer Vision**<br>도전 목표 · 일반 기준 | 주변 환경 인지 | 탐지 객체 **3종 이상**<br>Precision · Recall **80% 이상**<br>mAP@0.5 **75% 이상** | 임석헌 초안 (9/1) |
...
| `floating_ring(부유 고리)` | 고리 높이 **10.5 cm** · 폭 **42.5 cm** | **0%** [0~3.7] | 86% | **0%** | 0% | 100% |

**이 표는 세 가지 단서와 함께 읽는다.**

**`rails(턱)` 의 속도 추종 8% 는 100판 전체 기준이다.** 낙상 15판을 빼고 살아남은 85판만 보면 **7.1%** 다. 속도 오차는 스텝 평균이라 짧게 끝난 판일수록 작게 잡히고, 그래서 이 축은 낙상 판을 포함하면 실제보다 후하게 나온다. `rails(턱)` 는 낙상 판의 속도 추종 통과율이 **13.3%** 로 살아남은 판의 7.1% 보다 높다.

**`stepping_stones(디딤돌)` · `gap(틈)` · `floating_ring(부유 고리)` 의 직진성 100% 는 성과가 아니다.** 세 지형은 100판 전부가 통과선 3 m 앞에서 끝나 좌우로 벗어날 거리 자체가 없었다. 통과선을 넘고 이탈 기준도 만족한 판은 `rails(턱)` 44판 · `pit(구덩이)` 15판뿐이고, 직진 수치가 뜻을 갖는 것은 이 둘이다.
...
| 지형 | 병목의 크기 |
|---|---|
| `rails(턱)` | 속도 오차 평균 0.513 · 중앙 0.488 m/s (임계 0.25 의 2.05배) · 전진 중앙 3.02 m |
| `pit(구덩이)` | 전진 중앙 1.09 m (통과선 3 m 의 36%) · 시간 초과 97판 |
| `stepping_stones(디딤돌)` | 몸통 접촉 80판 · 전진 최대 1.32 m |
| `gap(틈)` | 몸통 접촉 79판 · 전진 최대 1.19 m |
| `floating_ring(부유 고리)` | 시간 초과 86판 · 전진 최대 1.22 m |
...
IsaacLab 은 난이도 0 에서 1 까지를 지형 파라미터에 선형으로 대응시킨다. 난이도를 `i/9` (i = 0~9) **열 칸**으로 나누고 **칸당 100판**을 돌린다. 칸당 100 은 2026-09-02 팀 결정이다. 30판은 95% 신뢰구간 폭이 24.4 포인트라 학습 전후를 구분하지 못한다.
...
| `floating_ring` 고리 높이 cm | 16.0 | 14.8 | 13.6 | 12.3 | 11.1 | 9.9 | 8.7 | 7.4 | 6.2 | 5.0 |
| `floating_ring` 고리 폭 cm | 25.0 | 28.9 | 32.8 | 36.7 | 40.6 | 44.4 | 48.3 | 52.2 | 56.1 | 60.0 |

**`stepping_stones(디딤돌)` 의 돌 폭과 `floating_ring(부유 고리)` 의 고리 높이는 난이도가 오를수록 값이 내려간다.** 디딜 자리가 좁아지고 고리 아래로 지나갈 높이가 낮아지는 방향이다.
