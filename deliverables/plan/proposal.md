...
**조건** · 6초 · 통과선 3 m · 1.0 m/s · 지형당 100판 (총 1,000판) · seed 42 · **난이도 0.5 고정** · **좌우 이탈 임계 1.5 m**
...
**층 2 의 경계값은 아직 없다.** 지금까지의 1,000판은 평가 설정이 `difficulty_range=(0.5, 0.5)` · `num_rows=1` 이라 **전부 난이도 0.5 한 점**에서 측정한 값이다. 난이도 격자를 아직 돌리지 않았으므로 **이동폭 목표를 지금 적지 않는다.** 격자 규격은 7-2 에 있다.
...
|---|---|---|---|
| **난이도 경계의 이동폭** | 경계값이 없다 |  | 실패 5종 각각 난이도 열 칸 × 100 에피소드 = **지형당 1,000판**. 지금까지의 1,000판은 난이도 0.5 한 점이라 경계를 만들지 못한다. 규격은 7-2 |
...
**조건** · 6초 · 통과선 3 m · 1.0 m/s · 지형당 100판 · seed 42 · 난이도 0.5 고정 · 좌우 이탈 임계 1.5 m (`#99` 2026-09-03 재측정). 물리값은 난이도 0.5 를 IsaacLab 보간식에 넣어 얻었다 (`mesh_terrains.py` · `hf_terrains.py` 원문 확인).
...
**`rails(턱)` 의 속도 추종 8% 는 100판 전체 기준이다.** 낙상 15판을 빼고 살아남은 85판만 보면 **7.1%** 다. 속도 오차는 스텝 평균이라 짧게 끝난 판일수록 작게 잡힌다. 낙상 판은 지속 시간 중앙값이 **4.08초**로 살아남은 판의 **6.00초**보다 짧아 오차가 낮게 잡히고, 그래서 낙상 판의 속도 추종 통과율이 **13.3%** 로 살아남은 판의 7.1% 보다 높다. **목표 기준은 전체 100판 중앙값 0.488 이 아니라 살아남은 85판의 0.502 를 쓴다.**

**`stepping_stones(디딤돌)` · `floating_ring(부유 고리)` 의 직진성 100%와 `gap(틈)` 의 97%는 성과가 아니다.** 세 지형은 100판 전부가 통과선 3 m 앞에서 끝나 좌우로 벗어날 거리 자체가 없었다. 통과선을 넘고 이탈 기준도 만족한 판은 `rails(턱)` 44판 · `pit(구덩이)` 15판뿐이고, 직진 수치가 뜻을 갖는 것은 이 둘이다.

**Wilson 구간은 이번에 생성된 지형에서 수행한 100판의 통과율에 대한 구간이다.** 평가 설정이 `num_rows=1` · `max_init_terrain_level=0` 이라 지형당 100판이 **동일하게 생성된 지형**에서 실행됐다. `discrete_obstacles(이산 장애물)` · `stepping_stones(디딤돌)` · `repeated_boxes(반복 박스)` · `repeated_cylinders(반복 원기둥)` 은 전역 난수를 사용하며, 이번 평가는 seed 42 에서 생성한 타일 하나를 대상으로 했다. 지형을 다르게 생성했을 때의 결과 변동은 이 구간에 포함되지 않는다(n = 1).

**같은 0% 가 같은 뜻이 아니다.**

| 지형 | 병목의 성격 | 병목의 크기 |
|---|---|---|
| `rails(턱)` | **속도 추종 부족.** 전진 성공률 51%, 속도 추종 성공률 8% | 속도 오차 평균 0.513 · 중앙 0.488 m/s (100판 기준 · 임계 0.25 의 2.05배) · 전진 중앙 3.02 m · 최대 4.91 m |
| `pit(구덩이)` | **전진 미달.** 전진 성공률 15% | 전진 중앙 1.09 m (통과선 3 m 의 36%) · 시간 초과 97판 · 15판이 3 m 를 넘었고 최대 5.14 m |
| `stepping_stones(디딤돌)` | **낙상** | 몸통 접촉 80판 · 전진 최대 1.32 m |
| `gap(틈)` | **낙상** | 몸통 접촉 79판 · 전진 최대 1.19 m · 3 m 를 넘은 판 0 |
| `floating_ring(부유 고리)` | **전진 미달.** 지형 형상과 정책의 영향을 추가 검증 | 시간 초과 86판 · 전진 최대 1.22 m (고리 안쪽 면 0.75 m · 바깥 면 1.175 m) |

**`rails(턱)` 을 제외한 네 지형의 전진 중앙값은 0.92 ~ 1.09 m 다.** 첫 장애물은 타일 중심에서 0.75 m 지점에 있다(`platform_width` 1.5 m). 네 지형의 중앙값은 통과선 3 m 에 미달하지만, 모든 판이 첫 장애물 앞에서 끝났다는 뜻은 아니다. `pit(구덩이)` 은 15판에서 통과선을 넘었다. `rails(턱)` 은 전진 중앙값 3.02 m 이며 속도 추종 부족이 주요 병목이다.
...
IsaacLab 은 난이도 0 에서 1 까지를 지형 파라미터에 선형으로 대응시킨다. 난이도를 `i/9` (i = 0~9) **열 칸**으로 나누고 **칸당 100판**을 돌린다. 칸당 100 은 2026-09-02 팀 결정이다.
...
맹라현이 MuJoCo 에서 같은 체크포인트로 턱 높이 4 cm 부터 18 cm 까지 열다섯 칸 · 칸당 100판으로 측정해 **경계를 13~14 cm** 로 냈다(`#139`). **우리 Isaac 측정과 어긋난다.** MuJoCo 는 10.5 cm 에서 90% 인데 Isaac 은 11.5 cm 에서 4% 다. 판정 조건도 다르다. MuJoCo 쪽은 20초 · 전진 4 m · 낙상 없음 · 몸통 접촉 없음이고 속도 추종 축이 없다. Isaac 쪽은 6초 · 통과선 3 m · 네 축 전부다.
