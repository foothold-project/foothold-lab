...
**Wilson 구간은 초기 조건의 분산만 덮는다.** 평가 설정이 `num_rows=1` · `max_init_terrain_level=0` 이라 지형당 100판이 **같은 지형 실현 한 벌** 위에서 돌았다. `discrete_obstacles(이산 장애물)` · `stepping_stones(디딤돌)` · `repeated_boxes(반복 박스)` · `repeated_cylinders(반복 원기둥)` 은 전역 난수를 써서 seed 42 아래 타일이 한 벌뿐이다. 구간은 지형 실현의 분산(n = 1)을 덮지 않는다.

**같은 0% 가 같은 뜻이 아니다.**

| 지형 | 병목의 성격 | 병목의 크기 |
|---|---|---|
| `rails(턱)` | **정책.** 넘기는 넘는다 | 속도 오차 평균 0.513 · 중앙 0.488 m/s (100판 기준 · 임계 0.25 의 2.05배) · 전진 중앙 3.02 m · 최대 4.91 m |
| `pit(구덩이)` | **정책.** 가끔 넘는다 | 전진 중앙 1.09 m (통과선 3 m 의 36%) · 시간 초과 97판 · 15판이 3 m 를 넘었고 최대 5.14 m |
| `stepping_stones(디딤돌)` | **낙상** | 몸통 접촉 80판 · 전진 최대 1.32 m |
| `gap(틈)` | **낙상** | 몸통 접촉 79판 · 전진 최대 1.19 m · 3 m 를 넘은 판 0 |
| `floating_ring(부유 고리)` | **기하.** 통과 경로가 없다 | 시간 초과 86판 · 전진 최대 1.22 m (고리 안쪽 면 0.75 m · 바깥 면 1.175 m) |

**통과선을 넘지 못한 넷은 전진 중앙값이 0.92 ~ 1.09 m 에 모여 있다.** 지형 타일의 `platform_width` 가 1.5 m 라 첫 장애물이 타일 중심에서 0.75 m 지점에 있다. **네 지형 모두 첫 장애물 앞에서 판이 끝난다.** `rails(턱)` 만 전진 중앙값 3.02 m 로 통과선을 넘어가며, 그래서 병목이 나머지 넷과 다르다.

**`floating_ring(부유 고리)` 의 전진 0% 는 기하에서 나온다.** 난이도 0.5 의 고리는 밑면 10.5 cm · 윗면 22.5 cm (두께 12 cm 고정)이고, 출발 발판을 사방으로 두른 닫힌 사각형이라 개구부가 없다. 밑으로 지나려면 10.5 cm 틈을 통과해야 하는데 정책의 기준 몸통 높이가 **0.40 m** 다(`UNITREE_GO2_CFG` 의 초기 자세). 위로 넘으려면 22.5 cm 단차를 올라야 한다. **이 지형의 목표는 통과율이 아니라 어느 난이도부터 통과가 성립하는지를 먼저 재는 것이다.**
...
**`floating_ring(부유 고리)` 는 난이도 축이 단조롭지 않다.** 난이도가 오르면 고리 밑 여유가 줄고(16.0 에서 5.0 cm) 고리 윗면도 낮아진다(28.0 에서 17.0 cm). 정책의 기준 몸통 높이가 **0.40 m**(`UNITREE_GO2_CFG` 의 초기 자세)라 밑으로 지나는 경로는 전 구간에서 성립하지 않고 넘는 경로만 남는다. 넘는 턱은 난이도 9 에서 17.0 cm 로 가장 낮은데, 우리 실측은 `rails(턱)` 11.5 cm 에서 전진 51% 라 **여기서도 5 cm 남짓 모자란다.** 따라서 이 지형은 **난이도가 높은 칸이 오히려 통과에 가깝다.** 담당(오흥재)이 격자를 돌릴 때 이 점을 함께 확인한다.
