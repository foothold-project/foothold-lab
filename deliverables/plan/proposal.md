...
| `gap(틈)` | 틈 폭 **27.5 cm** | **0%** [0~3.7] | **21%** | 0% | 0% | 100% |
...
**`stepping_stones(디딤돌)` · `gap(틈)` · `floating_ring(부유 고리)` 의 직진성 100%(이탈 임계 1.5 m 기준)는 성과가 아니다.** 세 지형은 100판 전부가 통과선 3 m 앞에서 끝나 좌우로 벗어날 거리 자체가 없었다. 통과선을 넘고 이탈 기준도 만족한 판은 `rails(턱)` 44판 · `pit(구덩이)` 15판뿐이고, 직진 수치가 뜻을 갖는 것은 이 둘이다.
...
| 판 | 날짜 | 무엇이 바뀌었나 |
|---|---|---|
| **v1.5** | 2026-09-03 | 7-1 KPI 를 실측 근거 위에 다시 씀(#154). 정본에 근거가 없던 목표치 6건(경계 25% 이상 이동 · 통과 5종 95% 유지 · `rails(턱)` 속도 추종 50% · 전진불능형 50% · 낙상형 생존율 60% · 실패 5종 통과율 30%)을 삭제하거나 축을 바꿔 교체. KPI 를 층 1(난이도 0.5 고정점 · 지금 확정)과 층 2(난이도 경계 · 09/12 확정)로 나눔. 목표마다 출처 열을 붙이고 Track B 아홉 값을 「도전 목표 · 일반 기준」으로 분류. 7-2 에 난이도 격자 규격 표와 `rails(턱)` 의 MuJoCo 교차 검증 불일치를 추가. 축 표에 읽는 단서 셋을 붙였다. `rails(턱)` 속도 추종 8% 의 낙상 판 편향(살아남은 85판은 7.1%) · 실패 3종의 직진성 100% 가 통과선 전에 끝난 결과라는 것 · Wilson 구간이 지형 실현 분산(n = 1)을 덮지 않는다는 것. `rails(턱)` 속도 오차 목표의 모집단을 살아남은 85판 하나로 통일했다(0.502 → 0.25 m/s). `floating_ring(부유 고리)` 의 0% 는 정책이 아니라 기하다. 난이도 0.5 의 고리가 밑면 10.5 cm · 윗면 22.5 cm 인 닫힌 사각형이고 정책의 기준 몸통 높이가 0.40 m 라 밑으로 지나는 경로가 전 구간에서 성립하지 않는다. 격자 표에 고리 윗면 행을 더하고 **난이도가 높은 칸이 오히려 통과에 가깝다**는 것을 적었다. `gap(틈)` · `pit(구덩이)` 도 출발점을 감싼 닫힌 링임을 밝혔다. Fall Rate 20% · 평지 직진성 90% · 칸당 100 에피소드 · 난이도 경계 축은 팀 결정이라 그대로 뒀다 |
