...
> 요지: 두 CSV 를 합쳐 FOOTHOLD 판 하나로 만든다. 대부분은 이미 있는 신호로 계산되고, 센서가 필요한 것은 12열뿐이다
> 상태: 초안 · 팀장 컨펌 대기
...
```
27 (판정)  +  8 (자세·제어)  +  16 (접촉)  +  5 (장애물)  +  5 (추적)  =  61열
별도로  obstacles_raw.csv  약 24열
```
