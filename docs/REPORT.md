...
> ### 방향 전환 공지 (2026-08-19 · 팀 전원 합의)
> 최종 실기 목표를 조정한다. **중간(9/30) = 시뮬레이션 강화학습 + 디지털 트윈 완결** · **최종(12/11) = 순정 보행 기반 SLAM·Nav2 실기 자율주행**. 우리 학습 정책의 실기 이전(저수준 배포)은 안전 장치(와이어 리깅 등) 부재와 난이도를 고려해 목표에서 제외한다. 아래 본문 중 Sim-to-Real 실기 이전 관련 절(5·7·8)은 이 결정 기준으로 v2 개정 예정.
...
| ② Parameter 변경 | `계단 높이` `경사도` `장애물 크기` `표면 거칠기` (지형 간격·배치·밀도 등 실험 목적에 필요한 다른 조건도 함께 검토) |
...
> **미팅 자문 필요** · 프로젝트 기간과 구현 난이도를 고려할 때 Proprioception 중심 Policy부터 구현할지, LiDAR/Depth 기반 Terrain 정보가 필수인지 결정해야 한다. 실제에서 구현하기 어려운 Observation은 학습 초기부터 제거하거나 대체할지도 함께 확정한다.
...
5. **자율주행 개발 시점**: SLAM/Nav2는 Sim-to-Real 이후가 적절한가, 일부 병렬 준비가 좋은가?
   **→ 확정 (8/19 오후)**: SLAM·Nav2가 확장이 아니라 **최종 실기 목표로 승격**. 조선대 커리큘럼 라인에서 ROS2 학습을 즉시 시작한다 (팀 우분투 24.04 멀티부트 구축 완료).
