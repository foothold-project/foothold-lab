...
| 기본 학습량 | 1,500 iteration ≈ **110분** (실측 기반 계산) |
...
├ 몸통이 얼마나 빨리 도는가 (3)
├ 중력이 어느 쪽인가 = 기울기 (3)
├ 어디로 가라는 명령 (3)          →   관절 12개의
├ 관절 12개의 현재 각도 (12)           목표 각도
├ 관절 12개의 현재 속도 (12)
├ 직전에 내가 한 행동 (12)
├ 몸통 속도 (3)          ← ⚠️ 실물에 이 센서가 없다
└ 발밑 지형 높이 187점 (187) ← ⚠️ 실물이 이걸 못 만든다
```
...
```
[Teacher]  특권 정보를 본다 → 아주 잘 걷는다 → 하지만 실물에 못 붙인다
                    │
                    │  증류 (시뮬 안에서, 지도학습)
                    ↓
[Student]  실물에서 읽을 수 있는 것만 본다
           (IMU 기울기·자이로 + 관절 엔코더 + 과거 이력)
           → Teacher 의 행동을 흉내 내도록 학습 → 실물에 붙는다
```
...
```
① 센서가 읽는다 (Go2 → ROS2 Humble → Orin NX)
     · IMU        : 몸이 얼마나 기울었나 + 얼마나 빨리 도나
     · 관절 엔코더 : 관절 12개의 각도 · 각속도 · 토크
                    (엔코더 = 관절이 지금 몇 도인지 재는 센서)
     · 직전 행동   : 방금 내가 뭘 시켰나
                              ↓
② 과거 이력을 이어 붙인다 (링버퍼 — 오래된 것부터 덮어쓰는 고정 크기 배열)
                              ↓
③ Student 신경망이 추론한다  ★ 여기서 지형을 역추정한다
                              ↓
④ 관절 12개 목표 각도를 뱉는다 → 모터 컨트롤러
                              ↓
             (0.02초 뒤 ①로. ★ 가중치는 절대 안 바뀐다)
```
...
```
        ┌───────────────────────────────────────┐
        │                                       │
   시뮬 학습 ──→ 정책 .pt ──→ ONNX ──→ Orin NX 배포
        ↑                                  │
        │                             실기에서 관찰
        │                                  │
        └─── 보상·지형·랜덤화 조정 ←── "이건 안 되네"
```

**로봇이 스스로 도는 게 아니다.** 한 바퀴에 학습 약 2시간 + 배포·시험이 붙는다.
...
```
[학습]  회색 무텍스처 · 4,096마리 · 렌더 끔    → 목적: 속도. 예쁠 필요 0
[렌더]  1마리 · 실사 지형 · Path Tracing       → 목적: 품질. 느려도 됨
```
...
```
스캔의 역할 = "실제 지형은 이런 통계를 갖는다"를 알아내는 것
              (돌 크기 분포 · 경사 분포 · 요철 주기)
                      ↓
학습 지형   = 그 통계로 절차 생성한 200개 타일 (매번 다름)
평가 지형   = 스캔 그대로 (한 번도 안 본 진짜 지형)
렌더 지형   = 스캔 그대로 + 고품질 텍스처
```
