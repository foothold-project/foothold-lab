...
> 분류: 운영
> 작성: 오흥재 · 2026-08-27 15:14
> 근거: 실측
> 요지: 외부 GPU 샌드박스 사양과 우리 용도 구분.
> 상태: 확정
> 판: v1.1
> 이슈: #86
...
| **V100 샌드박스** | 대규모 병렬 평가(MuJoCo/MJX), 3DGS 지형 재구성, sim-to-sim 교차 검증 |
...
| 항목 | 신청값 | 근거 |
|---|---|---|
| GPU | 상시 8장, 병렬 실험 기간 **최대 16장**(컨테이너 2개 × 8장) | 컨테이너당 8장이 상한이므로 컨테이너를 나눠 요청. 평가·3DGS·교차검증 세 갈래 동시 진행 |
...
> 이 네 줄은 **8/13 에 제출한 그대로**다. 고치지 않는다. 다만 그중 ~~정책 증류~~ 는
> 8/19 에 저수준 실기 이전을 목표에서 빼면서 필요가 없어졌다. 남은 세 갈래로 쓴다.
> 신청 용도와 실제 사용이 달라지는 것이 문제가 되는지는 `미확인` · 확인 필요.
...
여기서 도는 것은 평가·3DGS·교차검증 이지 Isaac Sim 이 아니기 때문이다(§2).
...
| 판 | 언제 | 무엇이 바뀌었나 | 근거 |
|---|---|---|---|
| **v1.1** | 2026-08-28 | 용도에서 정책 증류를 뺐다 (8/19 폐기). 신청서 원문 네 줄은 제출한 그대로 두고 주석을 달았다 | `DECISIONS` 08-19 |
