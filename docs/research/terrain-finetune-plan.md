...
> 근거: 실측
> 요지: 일반화 벤치마크 실패 5종을 5인이 하나씩 맡아 레시피를 찾고, 최종은 혼합 단일 런 1개로 합친다. `rails` 는 2026-09-03 에 낙상형에서 전진불능형으로 재분류됐고 처방도 함께 바뀌었다 (§1).
> 상태: 확정
> 판: v2.2
> 이슈: #59 #66
> 지형: 공통

> **[2026-09-15 정정] 아래 「실패 5종을 각 `proportion 0.1`」 배합은 1차 배포에서 실행되지 않았다.**
>
> 실제로 돌린 것은 **`gap` 하나만 `proportion 0.1`** 이고 **기존 험지 6종이 `0.9`** 다.
> 학습한 틈 폭은 A 가 `0.05 ~ 0.20 m` · B 가 `0.15 ~ 0.40 m` 다 (평가 기준 문서 모델 표 · 모델마다 SHA-256 기록). 근거는 팀장 확인이고
> 실행 당시 설정 파일과 체크포인트는 [`#428`](https://github.com/foothold-project/foothold-lab/issues/428) 에서 받는 중이다.
>
> 이 문서는 **계획**이다. 여기 적힌 수를 실행 결과로 인용하지 말 것.
> 정본은 [`DECISIONS.md`](../DECISIONS.md) 09-15 행이다.
...
| `rails` | **맹라현** | 0.5 m/s · 50 에피소드: 2% (1/50) / **36%** / 0.93 ± 0.32 m<br>**1.0 m/s · 100 에피소드: 4% / 85% / 2.64 m** (09-03 재측정) | ~~낙상 우세 (평균 2.1초) · 가장 빨리 넘어진다~~<br>-> **전진불능형** (09-03 재분류 · 아래 상자) | ~~관측 · 발 배치 보상~~<br>-> **보상 · 명령 커리큘럼** |
...
**배정의 근거**: 실패 모드가 두 종류라 처방이 갈린다. ~~낙상형 둘(`gap`·`rails`)~~ **낙상형(`gap`)** 은 관측과 발 배치, ~~전진불능형 둘(`pit`·`floating_ring`)~~ **전진불능형(`rails`·`pit`·`floating_ring`)** 은 보상 균형이다 (**`rails` 는 09-03 에 옮겨왔다 · 바로 아래 상자**). `stepping_stones` 는 v2에서 둘 다 나타난 유일한 지형이라 **두 처방의 비교 실험대**가 된다. 이 자리는 A/평가 주담당(오현민)이 맡는 것이 맞다. 판정 기준을 만든 사람이 두 처방을 같은 자로 재야 하기 때문이다.

> **2026-09-03 재분류: `rails` 는 낙상형이 아니다** (팀장 확정 · [`#66`](https://github.com/foothold-project/foothold-lab/issues/66) 코멘트)
>
> 위 배정의 근거는 **0.5 m/s · 50 에피소드** 위에 서 있었다. 같은 정책을 **1.0 m/s · 100 에피소드 · seed 42** 로 다시 돌리자 `rails` 만 성질이 바뀌었다.
>
> | 항목 | 0.5 m/s · 50 에피소드 | **1.0 m/s · 100 에피소드** |
> |---|---|---|
> | 생존율 | 36% | **85%** |
> | 전진 평균 | 0.93 m | **2.64 m** |
...
> | 종합 통과 | 2% | 4% |
>
> **속도를 올리니 안 넘어지고 앞으로 간다.** 100 에피소드 중 85 에피소드가 시간초과로 끝났고 67 에피소드가 2 m 를 넘겼다. 넘어져서 지는 것이 아니라 못 가서 진다.
>
> | 실패 모드 | 처방 |
> |---|---|
> | 낙상형 | 관측 · 발 배치 보상 |
> | **전진불능형** | **보상 · 명령 커리큘럼** |
>
> **그래서 맹라현의 처방이 §3 의 「낙상형」 줄에서 「전진불능형」 줄로 옮겨간다.** 전진 2.64 m 이고 통과선이 3 m 라 **0.36 m** 남았다. 실패 5종 중 통과선에 가장 가깝다 (`pit` 1.57 · `floating_ring` 1.06 · `stepping_stones` 1.05 · `gap` 0.92 m).
>
> 원자료 `sim/eval/results/20260903-rough10-1.0mps/`. 후속 진단은 [레일즈 진단](20260906-rails-speed-tracking-diagnosis.md) 이고, 그 문서 §7 이 **구간별로 관문이 갈린다**는 것까지 밀고 나갔다.
...
3. 바꿔볼 것 (**한 번에 하나**):
   ...
   - 보상 가중치. ~~낙상형(임석헌·맹라현)은 발 배치·접촉 페널티 쪽, 전진불능형(이민우·오흥재)은 전진 보상 쪽.~~ **09-03 재분류 반영: 낙상형(임석헌)은 발 배치·접촉 페널티 쪽, 전진불능형(맹라현·이민우·오흥재)은 전진 보상·명령 커리큘럼 쪽.** 오현민은 두 방향을 각각 돌려 비교한다
   - 에피소드 길이
...
레시피 5장을 합쳐 전체 혼합 cfg 를 만든다: **rough 계열 합계 0.5 + 신규 5종 각 0.1**

### 4-2. resume 대 scratch 비교

resume 런과 scratch 런을 1개씩 병행해 비교한다.

### 4-3. 비율 조정

특정 지형이 뒤처지면 그 지형의 `proportion` 상향 (비율이 곧 커리큘럼 압력)
...
| 판 | 언제 | 무엇이 바뀌었나 | 근거 |
|---|---|---|---|
| **v2.2** | 2026-09-13 | 머리에 `지형: 공통` 을 달았다. 이 문서는 다섯 지형 «전체» 계획인데 허브 보드가 `rails` 의 「레시피」 칸에 넣고 있었다. 공통 판정이 이슈 «번호» 만 세는데 머리말 이슈가 `#59 #66` 뿐이라 rails 하나만 맞았다. 본문은 다섯을 이름으로 다 다룬다 | 팀장 3회 지시 |
| **v2.1** | 2026-09-09 | **`rails` 를 낙상형에서 전진불능형으로 재분류**하고 §1 표·§3 보상 줄의 처방을 「보상 · 명령 커리큘럼」으로 바꿨다. 09-03 결정이 이슈 코멘트에만 있어 이 계획서를 먼저 읽는 담당자가 반대 처방을 실행하게 돼 있었다 | 팀장 09-03 확정 · #66 |
