...
2. ~~«NVIDIA 공식 체크포인트를 점수표에 처음 올린 팀» 이 된다~~ 는 접는다.
   RoboGauge 표는 45차원 체급이라 235차원인 NVIDIA 공식을 그대로 못 올린다.
   **대신 우리가 만드는 것이 다른 표다.** RoboGauge 는 *학습한* 지형 성능을 재고
   우리는 *안 배운* 지형을 잰다. 겹치지 않는 축이고 그것이 우리 고유 기여다.
3. **공개 4종을 우리 10종 지형에 태워 「대조군」으로 쓴다.** 이것이 이 조사의 새 쓰임이다.

   ~~우리 Student 가 완성되면 symmetry(0.7006)와 같은 체급에서 정면 비교한다~~ 는
   8/19 에 증류를 뺐으므로 성립하지 않는다. 45차원 정책을 만들지 않기 때문이다.

   대신 더 쓸모 있는 질문이 남는다. **우리가 실패한 5종에서 이들도 실패하는가.**

   | 결과 | 뜻 | 다음 행동 |
   |---|---|---|
   | 이들도 같은 5종에서 실패 | 미경험 지형 일반화는 아직 아무도 못 푼 문제다 | 파인튜닝 5종이 그 공백을 겨눈 것이 된다 |
   | 이들은 통과 | 우리 학습·관측 설계에 고칠 데가 있다 | 남의 레시피를 먼저 읽고 방향을 다시 잡는다 |

   **순위는 매기지 않는다.** 우리는 235차원(특권 관측을 본다), 이들은 45차원이라
   같은 자로 잰 것이 아니다. **어느 지형에서 깨지는가의 패턴만** 본다. 그 비교는
   관측 차원과 무관하게 성립한다. 산출물은 점수 순위표가 아니라 **실패 지형 지도**다.

   실행은 이미 가능하다. [관측 매핑](robogauge-observation-mapping.md) 이 확정이고
   4종 전부 로드·실행 검증을 마쳤다. 담당 `A/평가` · 이슈 #28.
...
| 판 | 언제 | 무엇이 바뀌었나 | 근거 |
|---|---|---|---|
| **v2.1** | 2026-08-28 | 45차원 4종의 쓰임을 정했다. 「우리 비교 상대」가 아니라 «미경험 지형에서 이들도 깨지는가»를 가리는 대조군이다. 순위 대신 실패 지형 지도로 낸다 | 팀장 지시 · #28 |
