...
가 네 축을 해부하고 1,000 에피소드를 축별로 재집계해 두었다. 이 문서는 그것을 대체하지
않는다.
...
그래서 그림 넷을 만들고, 각 그림 아래에 **그 숫자를 어느 파일 몇 행에서
...
같은 100 에피소드를 두 기준으로 재면 값이 이렇게 갈린다.
...
> **정본 1,000 에피소드는 재측정이 필요하다.** 그 기록은 20열이라 끝점으로 채점됐다
> (원장 09-09 행).
...
기존 500 에피소드 기록의 지형 인덱스가 전부 밀린다. 새 지형은 **맨 뒤에** 붙인다.
