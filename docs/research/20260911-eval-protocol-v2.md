...
그림 1 · 타일 배치와 통과선. 로봇은 8 m 타일 한가운데에서 출발하고 발판은 0.75 m 까지다
...
**그래도 3 m 가 기준으로 성립하는 이유는 이것입니다.** `rails` 의 가장 먼 레일이 2.88 m 이므로 **3 m 를 넘으면 넘어야 할 것을 다 넘은 것**입니다. 그보다 멀리 가라고 하면 남는 것은 지형 밖 평지뿐입니다.

**이 거리는 코드를 읽어 계산한 것이고 렌더해서 눈으로 재보지는 않았습니다.** `미확인` 으로 둡니다.

**6 초는 3 m 를 가는 데 필요한 시간의 두 배입니다.** 3 m 를 1.0 m/s 로 가면 3초입니다. 헤매서 두 배로 걸려도 3 m 만 넘으면 성공하도록 여유를 둔 것입니다.
...
![높이 스캔 값이 뜻하는 세 가지](../assets/visual/eval-v2-fig01.svg)
...
광선 한 줄기가 내는 값 
 ...
 +1 
 광선이 아무것도 못 맞힘 
 양수는 «바닥이 아래로 멀다» · 음수는 «위로 솟았다» · 닿지 않으면 학습은 +1, 평가는 -1 을 넣는다 
 그림 3-1 · 값 하나가 뜻하는 것. 양수는 바닥이 아래로 멀어진 것, 음수는 위로 솟은 것입니다

#### 3.2 gap 지형에서 광선이 어디를 비추나

난이도 0.5 에서 실제로 재 본 그림입니다. 재현은 sim/eval/probe_height_scan.py 입니다.

![gap 지형 위 광선 187개와 빗나간 22개](../assets/visual/eval-v2-fig02.svg)

gap 지형 · 광선 187개가 어디를 비추나 
 난이도 0.5 · 출발 직후 · 위에서 내려다본 그림 
 ...
 -1 
 아주 높이 솟음 · 장애물 
 그림 3-2 · 위에서 내려다본 gap 지형과 광선 187개. 빨간 점 22개가 틈 위로 나가 아무것도 못 맞힙니다
...
![floating_ring 단면에서 광선이 고리 윗면만 본다](../assets/visual/eval-v2-fig03.svg)

floating_ring · 광선은 고리 «윗면» 에서 멈춘다 
 난이도 0.5 단면 · 오른쪽이 진행 방향 · 가로세로 같은 축척 
 ...
 로봇이 못 아는 것 둘 
 ① 고리 «아래» 에 바닥이 있다는 것 ② 고리가 얼마나 «두꺼운» 지. 윗면 높이 하나만 잰다 
 그림 3-3 · 고리가 있는 자리에서 광선은 고리 윗면에서 멈춥니다. 그 아래 바닥은 값에 안 들어옵니다
...
**기본값은 안 바꿨습니다.** 팔을 안 주면 지금까지의 모든 실행과 같은 값으로 돕니다. 바꿔 버리면 쌓아 둔 41,800 에피소드와 새 숫자를 나중에 구별할 수 없습니다. 대신 **각 실행이 어느 규격으로 쟀는지 스스로 적게** 했습니다.
...
![학습 지형과 평가 지형의 달릴 거리와 틈 폭 비교](../assets/visual/eval-v2-fig04.svg)

연습한 지형과 시험 보는 지형이 이만큼 다르다 
 둘 다 난이도 0.5 · 옆에서 본 그림 · 가로 축척 같음 
 학습 · forward_gap 
 ...
 같은 난이도인데 
 달릴 거리는 절반 (1.50 → 0.75 m) · 틈은 2.2 배 (0.125 → 0.275 m) · 그리고 광선이 주는 값의 부호가 반대 
 그림 3-4 · 같은 난이도에서 달릴 거리는 절반, 틈은 2.2 배입니다
...
그림 2 · 방향 판정. 같은 에피소드를 두 방식으로 채점하면 결과가 갈린다
...
![규격 2 난이도별 성공률 세 모델 비교](../assets/visual/eval-v2-fig05.svg)

난이도를 올리면 어디서 무너지나 · 세 모델 
 규격 2 · 1.0 m/s · 칸마다 에피소드 100회 · 세로축은 성공률 % · 가로축은 난이도 
 ...
 B · 연습 틈 0.40 m 
 B 는 난이도 1.0 (틈 0.40 m) 까지 gap 100% 다. floating_ring 은 난이도가 오를수록 고리가 내려가 «쉬워진다». 이 지형만 축의 뜻이 반대다 
 stepping_stones 는 셋 다 0.2 에서 0 이 된다. 관측을 고쳐도 안 풀렸다. 뛰어야 넘는 지형이고, 위아래 출렁임 벌점(-2.00)이 그 동작과 부딪친다 
 그림 · 규격 2 로 다시 잰 난이도 곡선. 회색이 기준선, 주황이 A(연습 틈 0.20 m), 파랑이 B(0.40 m)
...
![정본 조건에서 잰 11개 지형 성공률](../assets/visual/eval-v2-fig06.svg)
...
0 
 ...
 정본 조건(난이도 0.5 · 1.0 m/s · 지형마다 100 에피소드)에서 잰 성공률. 축을 자르지 않았습니다.
...
![학습 평가 렌더 세 단계의 입출력과 소요 시간](../assets/visual/eval-v2-fig07.svg)

학습 · 평가 · 렌더는 각각 «다른 실행» 이다 
 하나를 돌린다고 다른 하나가 따라오지 않는다. 시간이 따로 든다 
 
 
 
 학습 
 입력 
 NVIDIA checkpoint.pt 
 출력 
 model_1500.pt 
 ...
 영상이 필요하면 «렌더를 따로 돌려야» 한다. 학습이나 평가가 영상을 남기지 않는다 
 그림 · 셋은 각각 다른 실행이다. 학습이 끝나도 영상은 안 나온다
...
![Go2 몸의 어디에 어떤 보상이 걸리는가](../assets/visual/eval-v2-fig08.svg)

보상이 몸의 «어디에» 걸리나 
 학습 실행이 저장한 설정에서 그대로 읽은 값 · 가중치가 큰 것부터 
 ...
 action_rate_l2 
 -0.01 
 명령이 프레임마다 크게 바뀌면 뺀다. 덜덜 떨림을 막는다 
 
 
 dof_pos_limits 
 0.00 
 관절 한계를 넘으면 벌점. 이 학습에서는 꺼져 있다 
 ...
 feet_air_time 
 +0.01 
 발이 공중에 0.5초 넘게 떠 있으면 준다. 종종걸음을 막는다 
 
 에피소드가 끝나는 조건 셋 
 
 
 base_contact 
 몸통이 땅에 닿으면 그 자리에서 끝. 실패 
 
 
 fell_below_terrain 
 지형보다 3 m 아래로 떨어지면 끝. 틈에 빠진 것 
 
 
 time_out 
 제한 시간이 지나면 끝. 실패 사유는 이것 아니면 base_contact 둘뿐 
 그림 · 보상 10개와 종료 조건 3개가 몸의 어디에 걸리는지. 사진은 실제 평가 렌더에서 오린 것입니다
...
4. 고른 판만 렌더한다 Random Seed과 환경 번호가 CSV 에 있으므로 **그 에피소드을 그대로 다시 만들 수 있습니다.**

#### 어느 에피소드를 찍나

| 고를 것 | 왜 |
|---|---|
| **가장 잘에피소드 하나** | 같은 조건에서 **되는 모습**이 어떤 것인지. 전진 거리가 가장 긴 성공 판 |
...
![그림 7 · 2.00초. 전진 1.10 m 에서 주춤이 세 번째로 들어갔고 전진·추종·방향이 아직 탈락 상태다](../assets/visual/eval-v2-shot05.png)

그림 7 · 2.00초. 전진 1.10 m 에서 주춤이 세 번째로 들어갔고 전진·추종·방향이 아직 탈락 상태다

![그림 8 · 4.00초. 전진이 1.11 m 에서 거의 멈췄다. 속도 0.02 m/s. 생존만 남았다](../assets/visual/eval-v2-shot06.png)

그림 8 · 4.00초. 전진이 1.11 m 에서 거의 멈췄다. 속도 0.02 m/s. 생존만 남았다
...
| 규격 | 에피소드 | 무엇 |
|---|---|---|
| **1** 결함 | 146,800 | 기준선 10종 · 난이도 스윕 · 기존 6종 스윕 · A · B |
| **2** 지금 | 64,400 누적 중 | 세 모델 정본 한 점 완료 · B 곡선 완료 · 기준선·A 곡선 진행 중 |
...
| **B** 2026-09-11 | 0.15 ~ 0.40 m | **완료** · 같은 조건 · 폭 두 줄만 다름 | c7612aef |
...
| 무엇 | 왜 필요한가 |
|---|---|
| **CSV 61열 통합** | 설계는 나왔다. 허벅지·종아리 접촉 센서를 붙이려면 한 번 다시 돌려야 한다 |
| **본 데이터 한 번에** | 180칸 = 18,000 에피소드. CSV 를 확정한 뒤에 돌려야 두 번 안 돈다 |
| ~~대표 영상 10종~~ | **했다.** 규격 2 로 112컷 · gallery-v1 |
