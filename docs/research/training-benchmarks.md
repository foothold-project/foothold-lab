...
2. **🔴 `collisionApproximateCylinders=true` 플래그는 공짜가 아니다.** 속도 2.4배(54,898 steps/s)를 얻지만
   **같은 300 iteration에서 보상이 1/4(13.94→3.54), 에피소드 길이가 절반(938.7→551.1 step)**으로 떨어진다.
   시드 고정 후에도 재현됐다. **본 실험에는 쓰지 말 것.**
...
**⚠️ 다만 이것도 완전한 비교는 아니다** — B는 같은 시간에 훨씬 많이 돌 수 있다.
```
22.07분 동안:  A = 300 iteration
               B = 약 700 iteration
```
**"같은 벽시계 시간을 주면 B가 A를 따라잡는가?"** → **B 700 iteration 측정 진행 중.**

### 실무 권고 (현 시점)

| 상황 | 권고 |
|---|---|
| **본 실험·최종 결과** | **플래그 쓰지 말 것.** 학습 품질이 명확히 낮다 |
| 빠른 반복이 필요한 탐색 단계 | 조건부 사용 가능 — **단 결과를 최종 수치로 쓰지 말 것** |
| 사전학습 체크포인트와 비교 | **반드시 플래그 없이** — NVIDIA 체크포인트는 기본 물리로 학습됐다 |
