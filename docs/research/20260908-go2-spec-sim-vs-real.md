...
![Go2 제원 그림](../assets/visual/20260908-go2-spec-diagram.svg)
...
| 발 반경 | **0.022 m** (USD 실측 · 구 collider) | **0.020 m** (공식 URDF `foot_radius`) | **두 출처가 2 mm 갈린다. 어느 쪽이 맞는지 여기서 판정하지 않고 둘 다 적는다** (§3-1 · `#332`) |
...
### 3-1. 발 collider 와 지형 메시 해상도

출처 = [`#332`](https://github.com/foothold-project/foothold-lab/issues/332) 코멘트 (2026-09-09 · 오현민 · 정적 접촉 시험). 제출 문서는 `inbox/oh/` 로 대기 중이다.

**발 collider 는 구(Sphere)다** `확인됨`. USD 실측에서 `{FL,FR,RL,RR}_foot/collisions/mesh_0` 이 넷 다 Sphere · 스케일 1.0 · 반지름 0.022 m · 지름 0.044 m 다. **캡슐이 아니다.**

| 출처 | 발 반경 | 무엇인가 |
|---|---|---|
| **우리 USD 실측** (`#332`) | **0.022 m** | 시뮬레이션이 실제로 도는 값 |
| **공식 URDF `foot_radius`** | **0.020 m** | 유니트리 공식 값 |

**어느 쪽이 맞는지 여기서 판정하지 않고 출처를 붙여 둘 다 적는다.** 2 mm 차이는 아래 걸침 경계를 4 mm 앞당긴다 (`2r` 이 0.044 에서 0.040 으로).
...
USD 0.022 m 로 계산하면 틈 2.5 cm 는 걸치고(예측 침하 3.9 mm) **틈 5 cm 부터는 기하학적으로 그냥 빠진다.** 학습으로 「밟는 법」을 배울 대상이 아니라 **밟을 자리를 피해야 하는 문제**라는 뜻이다. 다만 틈이 만들어지는 해상도(`hs` 0.025)에서도 **발의 12.5%가 2 cm 넘게 빠졌다** (최저 -16 cm). 기하학적으로 걸쳐져야 하는 폭에서 여덟 중 하나가 그대로 통과한다.

#### `hs` 0.1 에서는 미세 틈 지형이 «존재하지 않는다»

팀 정본 징검다리가 쓰는 해상도가 `hs` 0.1 이고, 그 격자에서 의도한 틈이 정수 픽셀로 절단된다 `확인됨`.

| 의도한 틈 | `hs` 0.1 에서 실현된 틈 | 발끝 z 중앙 | 접촉율 |
|---|---|---|---|
| 0.000 m | 0.000 m (0 px) | -0.0033 | 1.000 |
| **0.025 m** | **0.000 m (0 px)** | **-0.0033** | **1.000** |
| **0.050 m** | **0.000 m (0 px)** | **-0.0033** | **1.000** |
| 0.075 m | 0.100 m (1 px) | -0.2127 | 0.463 |
| 0.100 m | 0.100 m (1 px) | -0.2325 | 0.463 |
| 0.150 m | 0.100 m (1 px) | -0.2004 | 0.425 |
| 0.200 m | 0.200 m (2 px) | -0.1853 | 0.412 |

**틈 2.5 cm 와 5 cm 는 0 px 로 절단되어 연속 평면이 된다.** 세 지표가 틈 0 과 소수점까지 같다. 7.5 cm 와 15 cm 는 10 cm 로 스냅된다.

**그래서 `hs` 0.1 에서 잰 「미세 틈」 숫자는 미세 틈의 숫자가 아니다.** 미세 틈을 보려면 그 틈이 실제로 만들어지는 해상도(`hs` 0.025)로 다시 재야 한다.

**한계** · 정적 시험이다. 걷는 중에는 발이 속도를 갖고 들어가 걸침·빠짐 경계가 달라진다. 네 발이 함께 지지하므로 한 발의 침하는 나머지 셋의 지지에 좌우된다. 접촉 «점» 개수는 못 쟀다 (`ContactSensor` API 가 몸체별 합력만 준다). 지형이 전역 `np.random` 을 쓰므로 평가가 실제로 걸은 타일과 같지 않다.
...
| 14 | **발 반경 0.020 m 대 0.022 m 중 어느 쪽이 맞나** | 공식 URDF 와 우리 USD 실측이 2 mm 갈린다. 어느 쪽이 틀렸다고 할 근거가 없어 둘 다 적었다 (§3-1) | USD 자산의 출처 이력을 따라가거나 유니트리에 문의 |
...
`20260908-go2-제원-그림.svg` 를 같이 올렸다. 세 칸이다.
...
| 판 | 언제 | 무엇이 바뀌었나 | 근거 |
|---|---|---|---|
| **v1.1** | 2026-09-09 | **§3-1 신설.** 발 collider 가 구이고 반지름이 **USD 0.022 m 대 공식 URDF 0.020 m** 로 갈린다는 것을 출처를 붙여 둘 다 적었다(§3 표도 함께 고침 · §9 에 14번 추가). **`hs` 0.1 에서 틈 2.5·5 cm 지형이 0 px 로 절단돼 존재하지 않는다**는 것도 같이 옮겼다. 이 둘이 이슈 코멘트에만 있어 「0.02 m」와 「미세 틈 숫자」가 그대로 인용되고 있었다 | #332 |
