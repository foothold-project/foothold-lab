...
판 번호
  ...
  **사람이 판단하는 일**이고, 빌드 횟수와는 관계가 없다.
  ...
  v2.1.0  평가·갤러리 판 (2026-09-14) · 팀장 지시 12건 반영
          평가 영상 갤러리 신설 (전역바 · 지형 무리 거르개 · 3열 비교)
          연구 허브를 여섯 입구 «한 기준» 으로 · 지형 보드를 정본 실측으로
          평가 프로토콜 v2 정본화 · 일반화 벤치마크를 정본 기준 재작성 (문서 v2.6)
          새 관문 셋: 문단 폭 [3.41] · 그림 글자·선 [3.48] · 문서 숫자 대조 [3.86]
  ...
  v2.1.2  그림 여백과 영상 썸네일 (2026-09-14) · 팀장이 넷을 더 짚음
          그림 21장의 viewBox 가 제각각이라 일곱 장은 글자가 틀에 닿고
          반대쪽은 최대 321 이 비어 있었다. 사방 20 으로 맞췄다.
          영상 32개 중 19개에 썸네일이 없었다. 프레임 0 이 전부 검정이라
          폰에서 검정 네모로 떴다. 21개를 굽고 빌드가 자동으로 붙인다.
          새 관문 둘: 그림 여백 [3.474] · 영상 규격 [3.475]
  v2.1.3  화면으로 안 보고 고쳤다고 한 것들 (2026-09-14) · 팀장 6회 지적
          Go2 그림에 로봇이 빠져 있었다. `<img>` 로 불린 SVG 는 바깥 그림을
          안 부른다. 나는 SVG 를 «직접 열어» 보고 보인다고 했다.
          「같은 광선 22개…」 줄이 위 상자에 파묻혀 있었다 (네 번째 지적).
          `#####` 가 글자로 떴다. 제목 정규식이 h5·h6 를 안 받았고,
          인용 안 제목과 허브 카드 요지까지 합쳐 29곳이었다.
          새 관문 둘: 그림 바깥 자원 [3.473] · 렌더 검사 [3.476]
  v2.1.4  종합보고서와 평가 정본의 역할 분리 (2026-09-14 · #422)
  v2.1.7  종합보고서 레시피 서사 완성 · 한국어 정리 (2026-09-15)
          팀장 지시: 「다른 누군가가 봐도 이 레시피가 완성된 흐름을 잘
          파악할 수 있게」. 발행 전 검증(gpt-6-astra)을 여덟 차례 돌려
          약 150건을 고쳤다. 사실 오류 8건 · 한국어 90여 건 · 서사 빈틈.
          범위 그림 신설 (A 0.20 m < 평가 0.275 m < B 0.40 m).
          A·B 의 출발 체크포인트가 기록에 없어 미확인으로 적고 #428.
  v2.1.6  종합보고서를 서사 먼저로 · 죽은 화면을 잡는 관문 (2026-09-15)
          팀장 지적: 「자동화를 갖춰놨다고 했는대도 업데이트가 안되어있고
          죽어있는 문서가 되어있다」. 일정 허브가 9/15 에 「이번 주 W36」
          이라 말하고 D-day 는 빌드 때 박혀 매일 틀려지고 있었다.
          관문 [3.479] 가 화면이 말하는 «지금» 을 오늘과 대조한다.
          보고서는 결함 규격 자료를 인용하고 있었다. 관문 [3.4795].
          「도해」를 「그림」으로 138곳.
  v2.1.5  그림이 사이트 테마를 따라오게 (2026-09-14)
          팀장이 「SVG 컬러가 이전 컬러로 돌아갔다」고 잡았다. 페이지는
          다크인데 그림만 라이트였다. `<img>` 안의 그림은 격리 문서라
          data-theme 이 안 닿고 OS 설정만 보고 있었다. 테마마다 파일을
          굽고 토글이 갈아끼운다. 관문 [3.477].
          팀장: 「우리가 조사한 것들은 대부분이 종합평가서에 들어가야할
          내용들」 · 「맨 위에는 이번 보고서에서 우리가 얻은 것을 먼저」.
          보고서 첫머리에 「00 이번에 얻은 것」 다섯 줄을 두고 각 줄이
          근거 절을 가리킨다. 수는 원자료에서 읽는다.
          지형별 난이도 축 표와 보상 지도를 보고서로 옮기고, 정본은
          「결론은 보고서 어디」 한 줄로 가리킨다. 같은 그림 0종.
          절 번호가 04 와 08 이 겹쳐 있던 것도 고쳤다 (관문 9a · 9b).
