...
★ 이 관문이 «못 잡는 것»
  - **그 사람이 실제로 썼는지는 모른다.** 명부에 있는 이름인지만 본다.
    남의 이름을 적으면 통과한다. 그건 PR 리뷰가 본다.
  - 머리말의 `> 작성:` 한 줄만 본다. 본문에 적힌 이름은 안 본다.
    (규격 문서가 예시로 적어 둔 이름을 잡으면 관문이 헛운다)
  - 기계가 만든 문서는 사람 이름이 없는 것이 옳다. `NOT_PERSON` 참고.
"""
import io
import os
import re
