...
def main(site):
    ...
    #     «안 맞아도 된다» 는 뜻이 아니다.
    #   ★ 2026-09-13 실측. 지금 빠진 것이 «넷» 이 아니라 «둘» 이다.
    #     report-v1 은 전역바 · 테마 · 다크를 이미 갖고 있다. 빠진 것은
    #     검색 단추와 바닥 도장 둘뿐인데, 그 둘은 이 빌드가 «자기가 아는
    #     페이지» 에 주입하는 것이라 남의 생성기 산출물은 구조적으로 못 받는다.
    #     즉 그 생성기를 고치는 것만으로는 안 되고 주입 대상을 넓혀야 한다.
    #     그쪽 세션이 별도로 고치면 이 예외를 걷는다. lab#404 에 적어 뒀다.
    import build as _b
