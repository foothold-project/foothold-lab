...
# 전역바와 표지에 서지만 «이 빌드가 안 만드는» 목적지.
# (주소, 한글, 영문, 한 줄)
#
# ★ 2026-09-14. 갤러리가 잠깐 HUBS 에 있었다. 화면은 멀쩡했는데 HUBS 는
#   «이 빌드가 파일로 써내는 것» 의 목록이라 뜻이 어긋난다. 실측으로 이렇다.
#
#     hub3.build      RENDER 에 gallery 가 없어 «말없이» 건너뛴다
#     build.py:572    os.path.exists 로 막혀 «말없이» 건너뛴다
#     hubgen:934      주입 대상에 들어갔다가 실패해도 «말없이» 건너뛴다
#     ia.py 배정 보고  「갤러리 0」 이 영원히 찍힌다 (배정될 볼트 페이지가 없다)
#     build_hubs      os.path.join(VAULT,'/gallery/') -> C:/gallery/ 에 쓰려 든다
#
#   마지막 것은 지금 «안 돈다». v3 실패 대비용으로 남은 코드다. 그래서 오늘은
#   안 터진다. 안 터진다는 것은 지뢰가 없다는 뜻이 아니다 · v3 가 실패하는 날
#   빌드가 드라이브 루트에 파일을 쓰려 든다.
#   조용히 건너뛰는 자리가 셋이면 그 목록에 두지 않는 것이 맞다.
OUTSIDE = [
    ('/gallery/', '갤러리', 'Gallery',
     '평가 영상 144컷 · 지형과 속도별로 눈으로 보기'),
]

# 전역바·표지에서 서는 자리. 이 키 «다음» 에 온다.
# assets/gnav.html 이 연구 다음에 두고 있어 같은 차례로 맞춘다. 두 전역바의
# 차례가 다르면 페이지를 오갈 때 메뉴가 움직인다.
OUTSIDE_AFTER = 'research'
