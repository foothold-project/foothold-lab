...
★ 이 관문이 «못 잡는 것»
  - **자기 자신은 못 센다.** 관문을 세는 관문은 스스로를 셀 수 없다.
    `build.py` 도 뺀다. 안 빼면 자기를 읽다 무한히 돈다 (실제로 겪었다).
  - 「0개를 검사하고 통과」만 본다. 검사가 «틀린 것을 세는» 경우는 못 본다.
    개수가 0이 아니어도 엉뚱한 것을 세고 있을 수 있다.
  - `build.py` 가 `if not mod.fn(` 꼴로 부르는 것만 본다. 그 밖의 모양은
    안 보인다.
"""
import contextlib
import io
