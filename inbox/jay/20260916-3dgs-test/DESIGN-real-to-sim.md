...
**한 가지 전제가 더 깨졌다.** A 와 B 를 「서로 다른 횡단보도」로 보고 독립 검증을 설계했는데, 워커가 조감도를 대조하니 **중앙 파임 · 도색 돌출 · 노란 선 · 맨홀 배치가 닮아 같은 횡단보도를 다른 방향에서 찍은 것일 수 있다** `추측`. **「두 클립에서 재현됐다」고는 말할 수 있어도 「서로 다른 두 장소에서 재현됐다」고는 말할 수 없다.**

**정리 방법은 실측 한 번이다.** 그 횡단보도에서 **줄무늬 한 장의 폭과 바로 옆 간격을 같은 자리에서 둘 다** 재면 규격 인용과 시공이 동시에 갈린다.
