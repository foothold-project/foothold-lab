...
> 요지: 1차 PoC 는 파이프라인이 성립함을 보였고, 동시에 입력 영상이 제자리 촬영이라 지면의 절반 가까이가 지어낸 값임을 드러냈다. **다만 1 차를 버릴 것은 아니었다 · 2 차에 붙이면 같은 영역 관측률을 69 % 에서 86.5 % 로 올린다 (3-2 (라))**. 그러므로 복원 방법을 비교하기 전에 촬영을 다시 하고 축척 기준을 만든다.
> 상태: 검토중
> 판: v1.5
> 이슈: #430
...
#### (라) 그러나 1 차는 버릴 것이 아니었다 `확인됨` (2026-09-20)

**위 판정은 1 차를 «단독으로» 썼을 때의 것이다. 2 차에 붙이면 제 몫을 한다.**

`IMG_6698.MOV`(4K 원본)에서 뽑은 123 장을 2 차 A 에 합쳐 495 장으로 복원했다. 두 모델 좌표계를 **공통 카메라 372 개**로 맞춘 뒤(정렬 RMS 0.0017 m · 홀짝 교차검증 0.0017 m) **1 차 카메라 3 m 안의 같은 영역 10,663 칸**만 놓고 비교했다.

| 같은 영역 10,663 칸 | A 단독 | **A + 1 차** |
|---|---:|---:|
| 관측 비율 | 69.052 % | **86.505 %** |

**+17.45 퍼센트포인트.** 새로 관측된 칸 1,982 개 · 잃은 칸 121 개.

> **제자리 촬영이라 혼자서는 쓸모가 적던 1 차가, 2 차에 붙으니 그 지점의 관측 밀도를 올린다.**
> **「지어낸 값」이 줄어든 것이다.** 보간으로 메우던 칸이 실제 관측으로 바뀌었다.

**다만 넓어지지는 않았다.** 격자가 **50.2 x 9.35 m** 로 2 차 단일 클립과 거의 같은 구간이다. **1 차는 면적을 넓힌 것이 아니라 한 지점의 밀도를 올린 것**이고, 더 넓은 지형은 **같은 방향 재촬영**으로만 얻는다(7-2-1 절).

**그리고 「쓸 수 있다」는 잔차 비율과 관측 비율이 A 단독 수준이라는 뜻이지 실제 지면과 대조한 정확도 검증이 아니다** `미측정`. 잔차는 오히려 17.276 % 에서 **17.717 % 로 소폭 올랐다.**

#### (마) B 하나가 세 클립 판을 깨뜨렸다 `확인됨`

`image_deleter` 로 **B 256 장만 빼고** 495 장으로 같은 흐름을 다시 돌렸다. 재구성을 처음부터 안 한 것은 merge2 안에서 이미 이뤄진 **A-1 차 정합을 보존**하려는 것이다.

| 지표 | A 단독 372 장 | **A + 1 차 495 장** | 세 클립 751 장 |
|---|---:|---:|---:|
| 잔차 / 카메라 높이 | 17.276 % | **17.717 %** | **148.688 %** |
| 보행 관심영역 관측 | 62.690 % | **63.429 %** | **12.411 %** |
| 관측 칸 | 44,095 | 44,643 | 29,842 |

**B 를 빼자 A 단독 수준으로 돌아온다.** 세 클립 판의 148 % 잔차는 지형이 길어서가 아니라 **B 가 A 에 구속되지 않은 채 같은 파일에 들어 있었기 때문**이다(7-2-1 절).
...
| 방법 | 어떻게 | 무엇을 주나 | 지금 상태 |
|---|---|---|---|
| **(나) 노면 도색 규격 대조** ← **1 차 수단** | 영상에 찍힌 횡단보도 줄무늬 · 차선 폭 · 연석 높이를 법령 기준값과 대조 | **구간** (규격이 범위라 한 점이 안 나온다) | **2 차 촬영분에 이미 찍혀 있다** `확인됨` |
| **(다) 사후 실측** | 같은 장소에 다시 가서 연석 높이 · 도색 폭을 줄자로 잰다 | **점** (구간을 좁힌다) | 미수행. **촬영이 아니라 측정이다** |
| (가) 기준물 동반 촬영 | A4 · 1 m 줄자를 바닥에 놓고 함께 찍는다 | 점 | **하지 않는다**(6-2-4 절) |
...
| **2** | **횡단보도 줄무늬 폭** | **45 ~ 50 cm**. ~~줄무늬 간격은 폭의 1.5 배~~ **← 취소 · 아래 6-2-1-1 절** | 「도로교통법 시행규칙」 별표6 일련번호 532 (**도면 판독** · 수치 인용이 아니다) | 크기가 커서 원거리에서도 식별된다. **다만 「간격 1.5 배」로 자체 검산한다는 장치는 사라졌다** |
...
### 6-2-1-1. 「간격은 폭의 1.5 배」를 취소한다 `미확인` (2026-09-20)

**실측 네 경로가 전부 약 1.0 을 가리키는데 어느 하나도 1.5 쪽으로 가지 않는다.** 3DGS 세션 측정이다.

| 측정 | 평면 | 비(간격/폭) 중앙값 | IQR | 프레임별 검산 |
|---|---|---:|---|---:|
| A 구간 · 1 차 | 전역 | 0.950 | 0.921 ~ 0.972 | 0.927 |
| A 구간 · 재측정 | **국소** | **0.981** | 0.955 ~ 1.012 | 0.967 |
| B 구간 | **국소** | **0.963** | 0.930 ~ 1.004 | 0.957 |

**측정 쪽 혐의는 거의 지워졌다.** 전역 평면을 국소 평면으로 바꿔도 A 가 0.950 에서 0.981 로 **0.031 움직였을 뿐**이고, 1.5 와의 차이 0.519 중 **6 % 만 설명된다.** 국소 평면 적합 자체도 좋다(잔차/높이 A 0.77 % · B 0.41 % · 경고 기준 5 %).

**그래서 이 칸의 「1.5 배」를 `미확인` 으로 내린다.** 내가 적은 근거가 **「도면 판독」이지 수치 인용이 아니었고**, 2026-09-20 에 원문을 다시 찾았으나 **매뉴얼 PDF(70 쪽)에 그 표가 텍스트로 없어**(도면이 이미지) **재확인하지 못했다.**

> **읽은 것을 못 되읽으면 그것은 근거가 아니다. 취소한다.**

**무엇이 영향을 받고 무엇이 안 받는가.**

| | |
|---|---|
| **영향 없음** | **줄무늬 폭 45 ~ 50 cm 자체.** 축척 기준자로는 그대로 쓸 수 있다 |
| **영향 없음** | 지금까지의 축척 추정. 3DGS 세션이 **「축척은 걸지 않고 비만 낸다」**로 작업해 이 값이 축척에 안 섞였다 |
| **사라짐** | **비로 자체 검산하는 장치.** 실측 없이 「규격대로 시공됐나」를 볼 수단이 없어졌다 |

**한 가지 전제가 더 깨졌다. A 와 B 는 같은 횡단보도다** `확인됨` (팀장 확인 · 2026-09-20).

**세 클립은 전부 같은 공간이다.** 2 차 촬영은 「같은 장소를 촬영 설계대로 다시」였고, `IMG_6701` 과 `IMG_6702` 는 **같은 구간을 출발점과 끝점을 바꿔 찍은 것**이다. 우리 기록에도 이미 있었다 · `20260917-session-state.md` 4-4 절에 **촬영 시각이 17:29 와 17:31 로 2 분 차이**다.

> **그러므로 「두 구간에서 재현됐다」는 측정 재현성이지 규격 일반성이 아니다.** 한 횡단보도를 두 방향에서 잰 것이다. **이 데이터로는 다른 장소의 독립 검증이 애초에 불가능했다.** 「다른 구간을 재서 갈라 보자」는 설계 자체가 성립하지 않았다.

**남은 갈래는 둘이고 줄자 한 번이면 갈린다.** 그 횡단보도의 **시공**인가, **내 규격 인용**인가.

#### 이 한계는 횡단보도만의 것이 아니다 `확인됨`

**세 클립이 전부 같은 공간이므로, 이 촬영본으로는 어떤 규격값도 두 장소에서 독립 검증할 수 없다.**

| 기준자 | 같은 한계를 받는가 |
|---|---|
| 횡단보도 줄무늬 폭 · 간격 | **받는다** |
| 연석 높이 0.15 ~ 0.25 m | **받는다** |
| 황색선 폭 · 차선 폭 | **받는다** |
| 차로 폭 | **받는다** |

**8 절의 축척 구간(H1 · H3)도 한 장소 한 촬영에서 나온 값이다.** 「여러 기준자가 서로 맞는다」가 아니라 **「한 장소의 여러 기준자가 서로 맞는다」**이고, 그 장소의 시공이 규격을 벗어나 있으면 **전부 같이 틀린다.**

> **그러므로 다음 촬영 요청에 「다른 장소도 한 곳」을 넣는다. A4 · 줄자보다 값이 클 수 있다.**
> 지금 촬영본을 버리는 것이 아니다. **한 장소의 내부 일관성은 이미 확인됐고, 그것이 장소 밖으로도 가는지를 모르는 것**이다.

**정리 방법은 실측 한 번이다.** 그 횡단보도에서 **줄무늬 한 장의 폭과 바로 옆 간격을 같은 자리에서 둘 다** 재면 규격 인용과 시공이 동시에 갈린다. **촬영을 다시 할 일은 없다.**
...
### 6-2-3. 3DGS 세션이 이 절차를 고쳐 잡았다 (채택)

(0) 을 착수한 3DGS 세션이 설계 두 곳을 바꿔 제안했다. **둘 다 이 문서의 원안보다 낫다. 그대로 채택한다.**

#### (가) 단일 축척을 확정하지 않고 「허용 구간」을 구한다

원안의 결함은 이것이다. **규격이 범위(실선 10 ~ 15 cm)인데 측정값 하나로 축척 k 를 하나로 정하려 했다.** 정해지지 않는다.

고친 절차는 이렇다.

1. 측정마다 **허용 k 구간** `[규격 하한 / 측정값, 규격 상한 / 측정값]` 을 구한다
2. 그 선이 무엇인지에 대한 **가설 여러 개**를 세운다(둘 다 차선 · 하나는 중앙선 · 둘 다 중앙선 등)
3. 가설별로 구간의 **교집합**을 낸다
4. **교집합이 비면 그 가설을 기각한다**
5. 결과는 축척 하나가 아니라 **「k 구간과 그때의 폰 높이 구간」**

**이 방식이 옳은 이유: 가설을 데이터로 떨어뜨릴 수 있다.** 원안은 한 숫자를 내놓을 뿐 그것이 맞는지 틀리는지 말할 수 없었다.

#### (나) 연석 단차를 관측 칸만으로 다시 잰다

원안이 쓴 0.14 m 는 **격자 전체 히스토그램의 두 봉우리 간격**이라 보간 칸이 섞여 있다. 3-2 (다) 에서 보간이 지형을 지어낸다고 해 놓고 그 보간이 섞인 값을 기준으로 쓴 것은 앞뒤가 맞지 않는다.

고친 방법은 **연석선을 가로지르는 수직 프로파일을 관측 칸만으로 뽑는 것**이다. **6-2-2 절의 1.071 배는 이 재측정 결과가 나온 뒤에 갈린다. 그전까지는 예시일 뿐이다.**

#### (다) 첫 정찰 결과 (3DGS 세션 보고 · 서브픽셀 측정 전)

지면 평면으로 역투영한 287 프레임 조감 모자이크에서 황색 실선 둘 · 백색 실선 · 노면 문자가 선명하게 나왔다. 10 mm/px 거친 추정으로 황색 실선 폭이 이렇다.

| 측정 방법 | 값 |
|---|---|
| 면적 / 길이 | 0.128 m · 0.132 m |
| 거리 변환 | 0.160 m |

**두 방법이 25 % 차이 난다. 이 차이를 좁히는 것이 서브픽셀 측정(작업서 08)의 과제다.** 지금 값으로 가설 산술을 미리 해 보면 이렇다(측정값 0.130 m 가정).

| 가설 | 규격 | 허용 k 구간 | 그때 폰 높이 |
|---|---|---|---|
| 복선(중앙선) 또는 차선 실선 | 10 ~ 15 cm | 0.77 ~ 1.15 | **1.08 ~ 1.62 m** |
| 중앙선 단선 | 15 ~ 20 cm | 1.15 ~ 1.54 | **1.62 ~ 2.15 m** |

> **폰 높이 자체가 추가 제약이 된다.** 사람이 손에 든 폰이 2.15 m 높이일 수는 없다. 즉 「중앙선 단선」 가설은 폰 높이 쪽에서 상한이 잘린다. 이 교차 제약을 절차에 넣으면 가설을 더 좁힐 수 있다.

**주의: 위 산술은 거친 추정값 위에서 한 예시다.** 서브픽셀 측정이 끝나면 3DGS 세션의 값으로 대체한다.

### 6-2-4. A4 · 줄자를 동반 촬영하지 않는다 (팀장 결정 · 2026-09-18)

**이 문서의 앞 판은 「1 m 줄자 1 개 + A4 2 장」을 재촬영 필수 준비물로 적었다. 뺀다.**

근거 둘.

| | 무엇 |
|---|---|
| **탐색 결과** `확인됨` | 3DGS 세션이 2 차 촬영분 **전 370 프레임**을 훑어 A4 와 줄자를 **찾지 못했다**(`_out/shot2/REPORT-09.md`). 72 초의 흰 물체는 규격 미확인이라 기준물로 세지 않았다. 부재의 증명은 아니나, **있다고 치고 세울 계획은 못 된다** |
| **대체물이 이미 찍혀 있다** `확인됨` | 같은 보고서가 **두 클립 모두 연석과 횡단보도 가시 구간을 확인**했다. 횡단보도 줄무늬는 화면 폭의 1/3 이상을 차지한다. A4 긴 변 297 mm 보다 **훨씬 크다**(6-3 절의 픽셀 표에서 클수록 유리하다) |

> **그러므로 A4 를 놓으러 다시 나가지 않는다.** 노면 도색이 더 크고, 이미 찍혀 있고, 회수할 필요도 없다.

**대신 이 한계를 분명히 안고 간다.** 도색 규격은 **값이 아니라 범위**다(차선 실선 10 ~ 15 cm · 중앙선 15 ~ 20 cm · 횡단보도 줄무늬 45 ~ 50 cm). 그래서 도색만으로는 **축척이 한 점이 아니라 구간으로 나온다**(6-2-3 절의 가설별 허용 k 교집합).

**구간을 점으로 좁히는 것은 (다) 사후 실측 한 번이다.** 그것은 **촬영이 아니라 측정**이라 같은 장소에 가서 줄자로 재면 끝이고, **이번 촬영분을 그대로 쓴다.**

**한 가지는 지금 데이터로 바로 된다.** 횡단보도의 **줄무늬 폭 대 간격 비(약 1 대 1.5)는 축척과 무관**하다. 규격대로 시공됐는지를 실측 없이 검산할 수 있다. `REPORT-09.md` 는 이 대조를 **미측정**으로 남겼다. 이것이 다음 측정 항목이다.
...
**픽셀이 많을수록 좋다는 이 표가 곧 A4 를 뺀 근거다**(6-2-4 절). **횡단보도 줄무늬는 화면 폭의 1/3 을 넘어** A4 의 124 px 보다 훨씬 크고, 이미 찍혀 있다. 이 절은 이제 **기준물을 고르는 기준이 아니라 도색이 왜 더 나은지의 계산**으로 읽는다. (AprilTag · ArUco 마커는 표식이 필요한 데크 구간에서만 쓴다 · 7-3 절.)
...
| **기준물** | 없음 | **놓지 않는다.** 노면 도색(횡단보도 · 차선)과 연석이 기준물이다. **그 구간이 화면에 크게 들어오게 찍는다** | 6-2-4 절 |
...
> **이 그림은 이제 「무엇을 놓을까」가 아니라 「왜 도색이 더 나은가」를 읽는 자리다**(6-2-4 절). 놓는 기준물은 없다.

**대신 화면 안에 둘을 넣는다.** 성격이 다른 도색 둘(예: 횡단보도 줄무늬와 차선)을 서로 떨어진 구간에서 각각 크게 잡는다. 하나로 축척을 정하고 다른 하나로 검산한다(6-4 절).

### 7-5. 현장 체크리스트 (촬영 직전에 이것만 보면 된다)

**가져갈 것**

- [ ] **줄자** (바닥에 놓고 찍는 용도가 아니다. **연석 높이와 도색 폭을 재서 적는 용도**다 · 6-2-4 절)
- [ ] 데크 구간이 있으면 마스킹 테이프 (7-3 절)
- [ ] ~~A4 용지~~ **가져가지 않는다.** 노면 도색이 더 크고 이미 찍혀 있다

**촬영 전**

- [ ] 폰 **HDR 끄기**. 원본 파일을 그대로 받을 것 (재인코딩 금지)
- [ ] 해상도는 1080p 이상 · 프레임레이트 고정
- [ ] **횡단보도 줄무늬 폭과 간격을 줄자로 재어 적어 둔다** (규격 45 ~ 50 cm 범위를 점으로 좁힌다)
- [ ] **연석 높이를 줄자로 재어 적어 둔다** (성공 기준 2 의 정답지가 된다)
- [ ] 잰 자리를 **사진으로 한 장** 남긴다 (나중에 영상의 어느 구간인지 맞춰야 한다)
...
| 후보 | 어디서 | 왜 |
|---|---|---|
| A(2.5D 격자) · B(Poisson) | **Windows · conda `twin_env`** | 결과를 Isaac Sim USD 로 넘겨야 하고 Isaac Sim 은 Windows 쪽에 있다. B 는 CPU 라 GPU 도 안 쓴다 |
| **C(COLMAP dense)** | **WSL2 Ubuntu** ← **고침** | **Windows 에서 막혔다**(8-2-2 절) |
| **D(표면 복원)** · E(lingbot-map) | **WSL2 Ubuntu** | 저자들이 리눅스만 지원한다 |

### 8-2-2. C 가 Windows 에서 막혔다 `확인됨`

2026-09-18 · 3DGS 세션 실측. 1 차 undistorted 287 장에 `patch_match_stereo`(max_image_size 1600)를 걸었더니 30 초 만에 죽었다.

```
CUDA error at .../src/colmap/mvs/patch_match_cuda.cu:1743
the provided PTX was compiled with an unsupported toolchain.
```

드라이버 591.86 · RTX 5080(sm_120) · COLMAP 4.2.0 Windows 바이너리.

**같은 바이너리의 SIFT 특징 추출과 매칭은 GPU 로 잘 돈다.** 1 차와 2 차 검수에서 전부 성공했다. 곧 **CUDA 자체가 안 되는 것이 아니라 dense 모듈에서만 실패한다.** 원인은 `미확인` 이다(dense 만 PTX JIT 경로를 타는 것으로 `추측`).

> **이 구분이 중요하다.** 「CUDA 가 안 된다」면 Windows 경로 전체를 버려야 하지만, **「dense 만 안 된다」면 그것만 옮기면 된다.** A · B 와 SfM 은 Windows 에 그대로 둔다.
...
> **이 절은 두 번 정정됐다. 11-5-0(인지 세션 검증)과 11-5-1(기준선 실측)을 먼저 읽을 것.** 특히 **이 절이 물은 「굳는가」는 답이 반대로 나왔다**(11-5-1).

### 11-5-0. 인지 세션 검증 결과 · 이 브리프의 정정 (2026-09-17)

인지 세션이 소스에서 전수 확인하고 **틀린 것 둘, 빠진 것 셋, 그리고 문제 전제 자체와 어긋나는 실측**을 찾았다. 아래가 우선한다.

#### (가) 틀린 값 하나

**Spot 의 `lin_vel_x` 는 (-2.0, 2.0) 이 아니라 (-2.0, 3.0) 이다** `확인됨` (`spot/flat_env_cfg.py:67`). 같은 줄의 `ang_vel_z=(-2.0, 2.0)` 을 옆 칸에서 잘못 읽었다. `rel_standing_envs=0.1` 은 맞다.

#### (나) 「속도 0 을 본 적이 없다」는 절반만 맞다

`models/foothold-v1.agent.yaml` 에 `resume: true` · `load_run: nvidia_pretrained_source` 가 있고 `foothold-v1.json` 이 NVIDIA 체크포인트에서 시작했다고 적는다 `확인됨`. 그 상류 태스크는 `lin_vel_x=(-1,1)` · `rel_standing_envs=0.02` 다.

> **정책의 출발점은 속도 0 을 봤다. 우리가 1,500 iter 를 0.5 ~ 1.5 로만 미세조정하면서 잊은 것이다.**

**「본 적 없다」와 「봤다가 잊었다」는 실무상 다르다.** 후자면 **공짜 대조군**이 생긴다. NVIDIA 원본 체크포인트가 `C:/isaac/IsaacLab/.pretrained_checkpoints/.../checkpoint.pt` 에 있고 **두 체크포인트의 구조가 완전히 같다**(actor 235 → 512 → 256 → 128 → 12 · ELU · RNN 없음 · 정규화 없음) `확인됨`. 같은 프로브에 그대로 넣을 수 있다. 원본이 부드럽게 서고 우리 것이 굳으면 원인이 **우리가 바꾼 명령 분포 하나**로 좁혀진다.

#### (다) 마찰은 지형이 아니라 로봇에 걸려 있다

**9-1-1 절의 조언이 엉뚱한 손잡이를 가리켰다.** DR 이벤트는 `asset_cfg.name: robot` · `body_names: .*` 라 **무작위화 대상이 로봇 쪽 재질**이고 지형은 1.0/1.0 이다 `확인됨`. 지금 유효 마찰 0.8/0.6 은 로봇값 x 1.0 이다.

> **그러므로 v2 는 이미 있는 그 이벤트의 범위만 [0.5, 1.25] 로 넓히면 되고, 뽑힌 숫자가 곧 유효 마찰이 된다.** 지형에 0.5 를 넣어 0.4/0.3 을 만드는 쪽은 손댈 자리가 아니다.

#### (라) `make_consistent` 를 켜지 않으면 물리적으로 불가능한 조합이 나온다

`randomize_rigid_body_material` 은 static 과 dynamic 을 **독립으로** 뽑고 `make_consistent` 기본값이 `False` 다 (`events.py:155~`) `확인됨`. 지금은 양 끝이 같아 안 드러나지만 **둘 다 [0.5, 1.25] 로 넓히는 순간 동마찰 > 정마찰 이 섞인다.**

> **v2 는 `make_consistent=True` 를 반드시 같이 넣어야 한다.**

덧붙여 같은 함수가 버킷을 **강체 shape 마다** 독립 배정한다. 한 로봇의 네 발이 서로 다른 마찰을 가질 수 있다. 그리고 `mode: startup` 이라 **학습 전체에서 한 번만** 뽑는다(에피소드마다가 아니다).

#### (마) 명령 하한을 0 으로 내리면 지형 커리큘럼이 무뎌진다

`terrain_levels_vel` 의 강등 기준이 `distance < |command| x max_episode_length x 0.5` 다 (`curriculums.py:51`) `확인됨`. 명령이 0 에 가까우면 문턱도 0 에 가까워져 **그 환경은 영원히 강등되지 않고 승급 문턱(4 m)도 못 넘어 그 자리에 고인다.** 치명적이지는 않으나 저속 환경 비율이 늘수록 커리큘럼 진행이 느려진다. 설계에 반영해야 한다.

#### (바) 용어 정정 · 「규격 2」

11-2 절이 쓴 「`sim/eval` 규격 2」는 평가 프로토콜 판이 아니라 **높이 스캔 빗나감 처리 규격**이다 (`eval_generalization.py:248-252` 의 `_SPEC_VERSION`) `확인됨`. A/B 를 가르는 수단은 규격 2 가 아니라 지형별 원시 CSV 와 `run_manifest.json` 이다.

#### (사) 가장 중요한 것 · 문제 서술과 실측이 어긋난다

**이슈 #432 에 오현민 팀원이 2026-09-16 에 이미 잰 값이 있다.** 이 브리프에 없었다.

| 런 | 명령 | 결과 | 낙상 |
|---|---|---|---|
| flat05 | 0.5 | vx 0.474 · PASS | 0 |
| flat10 | 1.0 | **첫 정지 0.52 초 · 0.154 m.** 종료 평면속도 **0.068 m/s** 로 FAIL | 0 |
| turnwide | 1.0 + wz ±0.4 | wz 추종 0.337 / -0.318 · 종료 0.075 m/s 로 FAIL | 0 |

> **정지는 된다. 그것도 빠르다.** 1.0 m/s 에서 0.52 초 · 0.154 m 는 장애물 회피 제동거리로 오히려 짧다. 세 런 모두 **낙상 0** 이다.
>
> **안 되는 것은 둘이다.** ① 「서서히」가 아니라 급하게 멈춘다 ② 멈춘 뒤 0.068 m/s 로 계속 흐른다(정지 **유지** 실패).

**그러므로 이 브리프가 세운 전제 「장애물 앞 정지 · 우회를 하려면 감속이 먼저 돼야 한다」는 실측과 맞지 않는다.** 정지는 이미 되고, 못 하는 것은 **부드러움과 정지 유지**다. 이 구분이 v2 의 목표 함수를 바꾼다.

#### (아) 더 급한 축이 따로 있다 · 회전

우리 정책은 `ang_vel_z=(0,0)` · `lin_vel_y=(0,0)` 으로 학습됐다. **속도 0 만 분포 밖인 것이 아니라 회전 명령 전체가 분포 밖이다.**

그런데 **#434 가 최우선으로 잡은 것은 직각으로 꺾이는 ㄱ자 코스를 1.0 m/s 로 자율 주행하는 것**이다(NAV 관문 11/7). turnwide 의 wz 추종비 0.84 가 그 증거다.

> **「우회」는 감속이 아니라 회전 명령으로 한다. v2 후보에 `ang_vel_z` 복원이 빠지면 NAV 가 막힌다.**

#### (자) v2 판 구성 (인지 세션 제안)

v1 학습이 RTX 5080 한 대에서 **96 분 58 초** 였으므로(`foothold-v1.json` 실측) 세 판 추가가 약 5 시간이다. 섞어서 한 판으로 줄일 이유가 없다.

| 판 | 명령 분포 | 마찰 DR | 뜻 |
|---|---|---|---|
| v1 | 그대로 | 고정 | 기준선 (이미 있음) |
| **v2a** | 고침 | 고정 | 명령 분포 단독 효과 |
| **v2b** | 그대로 | [0.5, 1.25] | 마찰 단독 효과 |
| **v2c** | 고침 | [0.5, 1.25] | 상호작용 |

**재샘플링 10 초 → 4 초는 넣지 않는다.** 이 브리프도 `추측` 으로 적은 값이라 실험 칸을 쓸 근거가 약하다. v2a 가 여전히 느리면 그때 단독 후속으로 건다.

**판정은 두 축의 AND 다.** ① 정지 · 감속 프로브 통과 ② **회귀 방지: unseen10 난이도 0.5 세 속도에서 v1 점수를 안 잃을 것.** gap 과 pit 를 되돌리면서 정지를 얻는 것은 손해다.

---

**아래는 원래 브리프다. (가) ~ (자) 와 어긋나는 부분은 위가 우선한다.**
...
### 11-5-1. 기준선 실측 · 「얼음」은 얼음이 아니었다 (2026-09-18) `확인됨`

**팀장 지시로 학습 없이 두 정책을 같은 프로브에 넣어 쟀다** (`sim/eval/results/20260918-command-baseline/` · 커밋 bcc77d5 · 평지 · env 64 x 시나리오 4 x 정책 2 · seed 42). **이것이 이 주제의 정본이고, 11-5 와 11-5-0 (사) 보다 우선한다.**

| 관절 목표각 변화량 `action_delta_rms` (끝 1 초) | NVIDIA 원본 | foothold-v1 | 배수 |
|---|---:|---:|---:|
| stop (달리다 0 명령) | 0.0021 | **0.3370** | **164 배** |
| hold (처음부터 0 명령) | 0.0015 | **0.1258** | **81 배** |

**굳은 것이라면 0 으로 수렴해야 하는데 원본의 80 ~ 164 배다.**

> **v1 은 서 있으라는 명령에 쉬지 않고 새 목표각을 내면서 안정점을 못 찾는다. 굳어서 무너지는 것이 아니라 떨면서 무너진다.**

**그러므로 이 브리프가 물은 「정책 출력이 상수로 굳는가」는 답이 반대다. 처방이 갈린다. 출력 평활화가 아니라 자세 안정화 문제다.**

다만 `action_delta_rms` 하나로 원인을 확정하지는 못한다. **「굳었다가 아니다」는 말할 수 있고 그 반대는 아니다.** 원인을 좁히려면 접촉 · 발 미끄럼 · 자세각을 함께 봐야 한다.

함께 나온 것 둘.

| | NVIDIA 원본 | foothold-v1 |
|---|---:|---:|
| 정지 잔류속도 (끝 1 초) | 0.0021 | **0.0947** (44.7 배) |
| 정지 낙상 | 0/64 | **13/64** |
| 회전 낙상 | 4/64 | **64/64** · 7.28 초 전멸 · 롤 158.3 도 |
| 요레이트 추종비 `wz -0.5` | 0.615 | **-0.081** (부호 반대) |
| 저속 명령 0.20 실속도 | 0.077 | **0.187** (v1 이 낫다) |

**저속은 안 잃었다.** 「감속 · 저속이 문제」라는 이 절의 전제 중 저속 부분은 실측과 맞지 않는다. 남은 문제는 **정지 유지와 회전**이다. 자세한 것은 `inbox/jay/20260918-v2-command-restore.md` 0-2-2 절.
...
**이것을 「해소」라고 부르면 안 된다.** 인용을 검색 스니펫으로만 봤고 원문 페이지는 403 으로 열지 못했다. 3DGS 세션이 이 점을 지적했고 맞다.

> 12-3 절이 「해소」로 적혀 있는데 근거가 검색 스니펫이면 RESULTS 에는 아직 반영하지 않겠습니다.

**옳은 판단이다. RESULTS 에 반영하지 않는다.** 확인 방법은 정해져 있다. **proxy 를 켠 판과 끈 판으로 같은 프레임을 렌더해 비교한다.** 3DGS 세션이 12-2 의 조명 시험과 함께 돌린다. 그 결과가 나오기 전까지 이 절은 `추측` 이다.
...
| 순위 | 항목 | 내용 | 대응 |
|---|---|---|---|
| **1** | **비교 PoC 가 통째로 WSL2 에 걸렸다** | 표면 복원 여섯 가지 중 Windows 공식 지원이 **하나도 없고**(SuGaR 는 저자가 비호환 명시) `확인됨`, **그 안전망으로 잡았던 C(COLMAP dense)마저 Windows 에서 막혔다** `확인됨` (8-2-2 절). **안전망이 위험과 같은 것에 의존하게 됐다** | **WSL2 CUDA Toolkit 설치가 비교 PoC 의 단일 선행 조건이다**(pipeline 세션 몫 · 미완). 끝나면 **C 를 먼저 돌려 대조군부터 확보하고** 그다음 D 후보 하나를 시도한다. C 는 A · B 와 원리가 다른 유일한 대조군이라 D 보다 우선한다. D 는 하루 넘으면 중단 |
...
| 11 | NuRec proxy 렌더 규칙 | 검색 스니펫으로만 봤고 원문 페이지는 403 이었다. **v1.0 에서 이것을 「해소」로 적은 것은 과장이었다**(3DGS 세션 지적) | proxy 켠 판과 끈 판으로 같은 프레임을 렌더해 비교. 12-2 의 조명 시험과 함께 돈다 |
...
**「메시가 안 드러난다」는 해소되지 않았다.** v1.0 에서 해소로 적었으나 근거가 검색 스니펫이라 `추측` 으로 내렸다(12-3). 3DGS 세션이 실제 렌더로 확인한다.
...
| **8** | 「메시가 안 드러난다」의 유력한 설명을 찾았다. NuRec 이 상대 위치와 무관하게 항상 proxy 위에 렌더된다는 문장이 있다 | NVIDIA 문서 `추측` (원문 403 · 스니펫만 봤다) | **해소가 아니다.** 3DGS 세션이 proxy 켠 판과 끈 판을 렌더해 확인한다 |
...
| 판 | 언제 | 무엇이 바뀌었나 | 근거 |
|---|---|---|---|
| **v1.5** | 2026-09-20 | **1 차 촬영 평가를 고쳤다**(3-2 (라)·(마) 신설). 「제자리 촬영이라 절반이 지어낸 값」은 **단독으로 썼을 때**의 판정이고, 2 차에 붙이면 **같은 영역 관측률을 69.05 % 에서 86.51 % 로 올린다**(+17.45 %p · 새 관측 칸 1,982). 다만 **면적을 넓힌 것이 아니라 밀도를 올린 것**이고 정확도 검증은 `미측정` 이다. 그리고 **세 클립 판의 148 % 잔차는 B 하나 때문**임을 격리로 확인했다 · B 만 빼면 A 단독 수준으로 돌아온다 | 3DGS 세션 실측 |
| **v1.4** | 2026-09-20 | **6-2-1 의 「횡단보도 간격은 폭의 1.5 배」를 취소했다**(6-2-1-1 절 신설). 3DGS 세션 실측 네 경로가 전부 약 1.0 을 가리키고 어느 하나도 1.5 로 안 간다. 국소 평면으로 바꿔도 0.031 만 움직여 **평면 오차가 원인이 아니다.** 내 근거가 「도면 판독」이지 수치 인용이 아니었고 원문 재확인에 실패해 `미확인` 으로 내렸다. **줄무늬 폭 45 ~ 50 cm 와 지금까지의 축척 추정은 영향 없다**(비를 축척에 안 섞었다). 사라진 것은 **비로 자체 검산하는 장치**다. A·B 가 같은 횡단보도일 수 있다는 것도 적었다 | 3DGS 세션 실측 · 원문 재확인 실패 |
| **v1.3** | 2026-09-18 | **팀장 결정으로 A4 · 줄자 동반 촬영을 뺐다**(6-2-4 절 신설). 3DGS 세션이 2 차 촬영분 전 370 프레임을 훑어 A4 와 줄자를 찾지 못했고, 같은 보고서가 두 클립 모두에서 연석과 횡단보도를 확인했다 `확인됨`. 횡단보도 줄무늬가 화면 폭 1/3 이상이라 A4 보다 크다. 6-2 · 6-3 · 7-2 · 7-3-1 · 7-5 를 전부 고쳤다. **남는 한계를 명시했다** · 도색 규격이 범위라 축척이 구간으로 나오고, 점으로 좁히는 것은 재촬영이 아니라 사후 실측 한 번이다. **그리고 실측 둘로 더 고쳤다. (1) C(COLMAP dense)가 Windows 에서 막혔다** `확인됨`. `patch_match_cuda.cu:1743` 의 PTX 오류(RTX 5080 sm_120 · 원인 `미확인`)로 죽는다. 같은 바이너리의 SIFT 는 GPU 로 잘 도니 dense 모듈만의 문제다. 8-2-2 절을 신설하고 분담표에서 C 를 WSL2 로 옮겼다. **위험표 1 번을 다시 썼다** · C 를 D 실패 시의 안전망으로 적었는데 그 안전망이 같은 WSL2 에 의존하게 됐다. 비교 PoC 전체가 WSL2 CUDA 설치 하나에 걸렸고, C 가 A · B 와 원리가 다른 유일한 대조군이라 D 보다 우선한다. **(2) 11-5-1 절 신설** · 기준선 실측에서 「얼음」이 굳음이 아니라 **떨림**으로 나왔다(`action_delta_rms` 원본의 164 배 · 81 배). 이 브리프가 반대로 물었고 처방이 갈린다 | 3DGS 세션 실측 · 기준선 실측(bcc77d5) |
| **v1.2** | 2026-09-17 | **인지 세션의 검증으로 11-5 절을 정정했다**(11-5-0 신설). Spot 명령 범위 오독 · 「속도 0 을 본 적 없다」가 절반만 맞다는 것(NVIDIA 체크포인트에서 미세조정한 것이라 출발점은 봤다) · 마찰 DR 이 지형이 아니라 로봇에 걸려 있다는 것 · `make_consistent` 기본 False 문제 · 명령 하한 0 이 지형 커리큘럼을 무디게 한다는 것 · 「규격 2」 용어 오용. 그리고 **이슈 #432 의 실측이 문제 전제와 어긋난다**. 정지는 0.52 초에 되고 낙상 0 이며, 안 되는 것은 부드러움과 정지 유지다. 회전 명령 전체가 분포 밖이고 NAV(#434)가 그것을 먼저 필요로 한다 | 인지 세션 검증 |
| v1.1 | 2026-09-17 | 3DGS 세션의 피드백을 반영했다. **(1) 12-3 절의 과장을 정정했다.** NuRec proxy 렌더 규칙을 「해소」로 적었으나 근거가 검색 스니펫이라 `추측` 으로 내렸고, 실제 렌더 확인 전까지 RESULTS 에 반영하지 않는다. **(2) 6-2-3 절을 추가했다.** 축척을 단일 값으로 확정하지 않고 가설별 허용 k 구간의 교집합으로 구하는 방식과, 연석 단차를 관측 칸만으로 다시 재는 방식을 채택했다. 둘 다 3DGS 세션 제안이고 원안보다 낫다. **(3) 8 절 설치 순서를 WSL2 실측에 맞춰 고쳤다**(`HANDOFF-os-migration.md` 참조) | 3DGS 세션 피드백 · AI-WS01 실측 |
