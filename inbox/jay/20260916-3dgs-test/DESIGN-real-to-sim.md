...
| 판 | 언제 | 무엇이 바뀌었나 | 근거 |
|---|---|---|---|
| **v1.3** | 2026-09-18 | **실측 둘로 고쳤다. (1) C(COLMAP dense)가 Windows 에서 막혔다** `확인됨`. `patch_match_cuda.cu:1743` 의 PTX 오류(RTX 5080 sm_120 · 원인 `미확인`)로 죽는다. 같은 바이너리의 SIFT 는 GPU 로 잘 도니 dense 모듈만의 문제다. 8-2-2 절을 신설하고 분담표에서 C 를 WSL2 로 옮겼다. **위험표 1 번을 다시 썼다** · C 를 D 실패 시의 안전망으로 적었는데 그 안전망이 같은 WSL2 에 의존하게 됐다. 비교 PoC 전체가 WSL2 CUDA 설치 하나에 걸렸고, C 가 A · B 와 원리가 다른 유일한 대조군이라 D 보다 우선한다. **(2) 11-5-1 절 신설** · 기준선 실측에서 「얼음」이 굳음이 아니라 **떨림**으로 나왔다(`action_delta_rms` 원본의 164 배 · 81 배). 이 브리프가 반대로 물었고 처방이 갈린다 | 3DGS 세션 실측 · 기준선 실측(bcc77d5) |
