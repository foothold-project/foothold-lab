...
def read_cameras(model):
    ...
    if not cameras or any(camera["model"] != "PINHOLE" for camera in cameras):
    ...
    with open(model / "images.txt", encoding="utf-8") as stream:
        for line in stream:
            ...
            if int(fields[8]) not in camera_ids:
    ...
    def clip_name(name):
        ...
        return normalized.split("/")[0] if "/" in normalized else normalized[0]
    segments = [[i, i+1] for i in range(len(poses)-1)
                if clip_name(poses[i][0]) == clip_name(poses[i+1][0])]
    length = float(sum(np.linalg.norm(centers[j]-centers[i]) for i, j in segments))
    ...
    return centers, downs, {"camera": cameras[0], "camera_records": cameras,
                            "camera_count": len(cameras), "image_count": len(poses),
                            "trajectory_segment_indices": segments,
                            ...
                            "trajectory_sort": "image filename order; no segments across clip boundaries"}
...
def distance_to_path(xy, cameras_xy, segments=None):
    minimum = np.full(len(xy), np.inf)
    if segments is None:
        segments = [(i, i+1) for i in range(len(cameras_xy)-1)]
    for i, j in segments:
        start, end = cameras_xy[i], cameras_xy[j]
        vector = end-start
...
def make_grid(points, camera_centers, cell, args):
    ...
    path_distance = distance_to_path(centers, camera_centers[:, :2], args.camera_segments).reshape(ny, nx)
...
def draw_heightmap(grid, cameras, output, segments=None):
    ...
    for panel in panels:
        if segments is None:
            ...
            for i, j in segments:
                cv2.line(panel, tuple(pixels[i]), tuple(pixels[j]), (0, 80, 255), 3, cv2.LINE_AA)
...
def arguments():
    ...
    owned_mesh_roots = [ROOT/"_out/shot2pipe/mesh", ROOT/"_out/shot2pipe/merge/mesh", ROOT/"_out/shot2pipe/merge2/mesh"]
    if not any(args.out.is_relative_to(path.resolve()) for path in owned_mesh_roots):
        parser.error("Output must remain inside owned shot2pipe mesh directories")
    return args
...
def run(args, stats):
    ...
    with stage(stats, "03_cameras"):
        ...
        args.camera_segments = stats["cameras"]["trajectory_segment_indices"]
        mean_down = np.mean(downs, axis=0)
    ...
    with stage(stats, "11_heightmap"):
        stats["heightmap"] = draw_heightmap(grid, cameras_m, args.out/"ground_heightmap.png", args.camera_segments)
