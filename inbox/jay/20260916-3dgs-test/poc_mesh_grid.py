...
def make_grid(points, camera_centers, cell, args):
    ...
    walking = (distance_to_path(centers, camera_centers[:, :2]) <= args.walk_radius_m).reshape(ny, nx)
    ...
    observed = counts >= args.min_points
...
def fill_grid(grid, args):
    ...
    fill = (~observed) & (distance <= args.fill_max_m)
    empty = (~observed) & (~fill)
...
def arguments():
    ...
    parser.add_argument("--ply", type=Path, default=ROOT/"_out/splat/splat.ply")
    parser.add_argument("--model", type=Path, default=ROOT/"_out/colmap/undistorted/sparse_txt")
    parser.add_argument("--out", type=Path, default=ROOT/"_out/mesh")
    defaults = {"min_opacity": .1, "max_scale_m": .3, "camera_height_m": 1.4, "cell": .05,
    ...
    for name, default in {"min_points": 3, "ransac_iterations": 2000, "hist_bins_per_path": 50,
                          "fill_k": 4, "median_size": 3, "face_limit": 100000, "seed": 42}.items():
    ...
    if not 0 < args.face_limit <= 100000 or not 0 <= args.curb_prominence_fraction <= 1:
        parser.error("face_limit must not exceed 100000; invalid curb prominence")
    args.ply, args.model, args.out = args.ply.resolve(), args.model.resolve(), args.out.resolve()
    if not args.out.is_relative_to((ROOT/"_out/mesh").resolve()):
        parser.error("Output must remain inside the owned _out/mesh directory")
    return args
...
def run(args, stats):
    ...
    with stage(stats, "06_transform_and_scale_filter"):
        ...
        keep_z = (points_m[:, 2] >= args.z_min_m) & (points_m[:, 2] <= args.z_max_m)
    ...
    stats["acceptance"] = {"script_completed": True, "mesh_exists": (args.out/"ground.obj").is_file(),
                           "face_limit": len(mesh.faces) <= 100000, "all_triangles": mesh_info["all_triangles"],
