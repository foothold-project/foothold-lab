...
**1 번의 근거 (실측)** `확인됨`. 팀장 기준선 `2026-08-11_20-32-58` 의 파일 시각이다.
...
다만 `gap_training/` 은 IsaacLab 저장소 안이라 거기 커밋하면 NVIDIA 업스트림과 엉킨다. **우리 저장소 `sim/` 아래에 정본을 두고 IsaacLab 으로 복사하는 쪽**이 맞아 보이나, 배치는 팀장 결정 사항이다.
