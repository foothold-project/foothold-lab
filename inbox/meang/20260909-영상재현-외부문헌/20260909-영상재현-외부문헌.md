...
**Go2 는 articulation 이고 우리 지형에 천도 연체도 없다. 보장 안쪽이다.**
...
**우리 환경이 `ManagerBasedRLEnv` 라 이 자리가 그대로 우리 자리다.**
...
**우리 평가는 판마다 `reset` 을 한다. 경고에 해당하는 자리다.**
...
**#6609 가 특히 나쁘다.** 자세는 바꿨는데 렌더는 안 바뀐 채로 영상이 나오고
**에러가 나지 않는다.**
...
**하나. 고르기 «전» 에 다 찍어야 한다.** 지금 찍어 둔 mp4 평균이 3.78 MB 이니
1,000판이면 **약 3.7 GB** 이고 실제로 보는 것은 열 개쯤이다.
