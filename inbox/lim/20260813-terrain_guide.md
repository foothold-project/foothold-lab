작성 2026-08-13
요지: Isaac Lab v2.3.2 Terrain 전체 정리 및 Unitree Go2 커스텀 험지 설계 가이드

> 대상: Isaac Lab **v2.3.2**, Unitree Go2 custom rough-terrain locomotion 학습  
> 기준: Isaac Lab v2.3.2 공식 API 및 공식 GitHub 소스  
> 작성일: 2026-08-13
...
험지 정의 및 관찰·측정
        ↓
험지 핵심 특징 분해
(높이 형상, 장애물, 간격, 재질/마찰, 변형성, 동적 요소)
        ↓
기본 제공 terrain으로 표현 가능한 요소를 우선 조합
(TerrainGeneratorCfg.sub_terrains + 비율 + 난이도 curriculum)
        ↓
마찰·외란·센서 오차 등은 terrain 형상과 분리하여 물리 재질/EventCfg로 모델링
        ↓
Go2 baseline을 같은 평가 조건에서 측정
        ↓
핵심 특징이 여전히 표현되지 않는가?
   ├─ 아니오 : 기존 조합과 파라미터를 개선
   └─ 예 : 새 terrain 함수 또는 외부 USD/mesh/별도 물리 모델 구현
        ↓
학습 → 고정 평가 세트 검증 → sim-to-real 범위 조정
```
...
## 2. v2.3.2 기본 제공 procedural terrain 전체 목록
