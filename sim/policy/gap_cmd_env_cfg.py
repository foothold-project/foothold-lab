...
**뒤 둘만 되돌린다.** 로봇은 여전히 틈을 향해 서지만 멈추고 돌 줄 알게 된다.
...
두면 클립 상하한이 둘 다 0 이라 **요레이트가 언제나 정확히 0** 이 된다.
heading 을 켜도 로봇은 안 돈다. 그래서 둘을 함께 움직인다.

곁들여 알아 둘 것이 하나 있다. `rel_heading_envs = 1.0` 이면 **모든** 환경의
요레이트가 heading 오차에서 나온다. D 는 「직접 지시된 요레이트」를 한 번도
학습하지 않는다. NVIDIA 원본도 똑같으므로 「되찾는 것」이라는 성격에는 맞지만,
회전 프로브가 요레이트를 직접 주입하면 **학습 분포 밖**을 재게 된다. 결함이
아니라 재는 쪽이 알고 있어야 할 조건이다.
