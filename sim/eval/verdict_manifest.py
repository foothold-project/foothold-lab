...
# 축 2 를 «몇 점에서» 넘어야 하나. 2026-09-23 에 3/4 에서 4/4 로 올렸다.
AXIS2_POINTS_REQUIRED = 4
AXIS2_POINTS_TOTAL = 4
AXIS2_POINTS_CHANGED = {
    "when": "2026-09-23",
    "from": 3,
    "to": 4,
    "why": ("우리는 체크포인트를 «하나» 골라 배포한다. 넷이 다 좋아야 "
            "「아무거나 골라도 된다」가 선다. 셋만 요구하면 나쁜 하나를 "
            "«평가 결과를 보고» 골라내게 되고 그것은 체리피킹이다"),
    # CRITERIA v1.2 4 절 2 번 · 뒤집히는 판정을 «전부» 찾아 적는다.
    # 9/9 를 «한 점에서라도» 낸 후보가 없어서(최고가 D 의 8/9 · v2b 의 7/9)
    # 3/4 든 4/4 든 결과가 같다. 그래서 비어 있다.
    "flipped": [],
    "source": "inbox/jay/20260923-lineage/CRITERIA.md v1.2 1 절",
}

# 최종 성공 기준 (팀장 확정 · 2026-09-23 · CRITERIA v1.2 1 절).
# 세 줄을 «다» 만족해야 한다.
FINAL_CRITERIA = {
    "axis1": "네 체크포인트 «전부» 에서 foothold-v1 대비 진짜 하락 0",
    "axis2": "네 체크포인트 «전부» 에서 %d/%d" % (
        AXIS2_GATE_CELLS, AXIS2_TOTAL_CELLS),
    "repeat": "다른 장치에서 한 번 더 · 위 둘을 다시 만족",
}

# CRITERIA v1.2 7-2 절 · 판정에서 «빼는» 지형.
# 열 정책 전부 0 % 이고 NVIDIA 원본도 0 % 다. 빼지 않으면 어떤 정책도
# 통과할 수 없다. **빼는 것은 «판정» 에서이고, 보고에서는 첫 줄에 남긴다.**
EXCLUDED_FROM_VERDICT = ("stepping_stones",)
EXCLUDED_WHY = ("정책 쪽 시도로는 아무도 못 넘었다 · 그래서 다음은 "
                "신경망 쪽을 본다 (「불가능하다」도 「아직 안 배웠다」도 "
                "아니다 · CRITERIA v1.2 7-2 절)")
