...
def check_bench(t, bad):
    ...
    **배포본이 그 판인가** 와 **고쳐 놓고 되살아나는 것** 만 본다.
    ...
    for pat, label, why in [
        ...
        # ★ 2026-09-14. 여기 「일일 청구」 검사가 있었다. **뺐다.**
        #   팀장이 같은 날 「비용 관련해서는 이제 빼버려」 라고 확정해
        #   `AGENTS.md` · `WORKFLOW.md` · `AUTOMATION.md` 의 예산 금액 금지를
        #   Go2 대여 금액으로 좁혔다. 그런데 **내가 쓴 이 검사만 안 고쳐**
        #   라이브 검사가 「되살아났습니다」로 섰다. 문서는 맞았고 관문이 낡았다.
        #
        #   규칙이 사는 자리를 셀 때 **내가 방금 만든 검사도 그 자리 중 하나**다.
        #   이번에 넷을 고치고 다섯째를 빠뜨렸다.
        #
        #   지금 살아 있는 금액 규칙은 Go2 대여 금액 하나뿐이고, 그것은
        #   `web/_build/scan.py` 가 이미 본다. 여기서 두 번 보지 않는다.
        n += 1
