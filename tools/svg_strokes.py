...
근거: 실측 (그림 15장의 stroke-width 전수 · 적용 전후 개수 대조)

## 왜 있나

2026-09-13 에 팀장이 프로토콜 페이지를 보고 「border 다름」을 지적했다.
재어 보니 그림 15장에 선 굵기가 **11가지** 쓰이고 있었다.

    1 · 1.5 · 1.6 · 1.7 · 1.8 · 2 · 2.2 · 2.6 · 2.8 · 3.4 · 6

같은 역할(가는 선 · 보통 · 강조)인데 그림마다 값이 다르다. `curves` 계열은
...
## 자리가 셋이다 (이걸 찾는 데 세 번 걸렸다)
    ...
    deliverables/plan/assets/   ← 제안서 그림의 «원본»
    web/assets/                 ← 빌드가 위에서 복사해 온 것
    docs/assets/visual/         ← 연구 문서 그림의 원본
...
#   6      fig06 의 굵은 띠         (한 그림 안에서만 쓰는 특수값)
#
...
def main(root, write):
    ...
    print('  그림 %d장 손댐 · 옮긴 선 %d개 · 받은 쪽 증가 %d개'
