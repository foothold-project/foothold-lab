...
> 요지: `<img>` 안의 도해는 OS 설정만 보므로, 테마별 파일을 만들어 토글이 갈아끼운다
> 상태: 확정

## 무엇이 틀렸었나

팀장이 「웹페이지 들어가면 SVG 컬러가 이전 컬러로 돌아갔다」고 했다.
실제로 봤다. **페이지는 다크인데 도해만 라이트로 남아 있었다.**

까닭은 자리가 둘인데 한쪽만 있었기 때문이다.

    사이트가 테마를 정하는 법    `<html data-theme="dark">`  (토글 버튼)
    도해가 테마를 정하는 법      `@media(prefers-color-scheme:dark)`  (OS 설정)

`<img src="x.svg">` 로 넣은 도해는 **격리된 문서**라 부모 페이지의
`data-theme` 이 안 닿는다. 그래서 도해는 OS 설정만 본다. OS 가 라이트인
사람이 사이트를 다크로 바꾸면 도해만 라이트로 남는다. 그 반대도 같다.
...
도해를 **테마마다 한 장씩** 만들고, 무엇을 볼지는 페이지가 정하게 한다.
...
`check()` 가 배포본을 다시 잰다. `<img>` 로 불리는 도해마다 다크 짝이
...
def paint(text, palette):
    """그 도해를 한 테마로 못박는다. media query 는 지운다.
...
def write_pair(path):
    """한 도해에서 라이트본과 다크본을 만든다. 돌려주는 것은 (바꿨나, 말)."""
...
def themeable(path):
    """이 도해가 테마 짝을 가질 수 있나. 파일을 읽어 판정한다."""
...
def mark_pages(site):
    """도해마다 표를 붙인다. 돌려주는 것은 (짝 표시 수, 판 표시 수, 판 목록).

    `data-themed`  테마 짝이 실제로 파일로 있는 도해. 토글이 갈아끼운다
    `data-plate`   짝을 못 만드는 도해. 다크에서 흰 판 위에 올린다
    ...
    for r in figs:
        ...
        if themeable(p) and os.path.isfile(os.path.join(site, dark_name(r))):
...
def _figs_in_pages(site):
    """페이지들이 `<img>` 로 부르는 도해를, 사이트 기준 «경로» 로 돌려준다.
...
def check(site, _unused=None):
    ...
    print("  페이지가 부르는 도해 %d종 · 짝 있음 %d · 밝은 판 %d · "
