...
**이 내용이 된 때** 2026-09-23 16:47 · 감시 스크립트 (평가를 거는 판)
...
막힌 것   [0-b] v2b-s 가 학습 중 죽었다. 분기표에 «학습 실패» 칸이 없다. 사람이 정해야 한다. [0-a] v2b-r 은 계속 돈다
다음      없음. 「학습이 죽었다」는 분기표에 없는 결과다
          (막는 것: 팀장 판단 · 같은 시드로 재시도할지, 다른 시드로 갈지, [0-b] 를 버릴지)
```
